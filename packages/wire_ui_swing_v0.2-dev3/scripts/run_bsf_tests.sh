...
if [[ -n "${WIRE_UI_SERVER_SRC:-}" && -n "${ALCHEMY_OBJECTS_SRC:-}" && -n "${OOREXX_CRYPTO_SRC:-}" ]]; then
  ...
  rexx "$ROOT/integration/test_server_v011_contract.rex"
else
  echo "SKIP server v0.11 contract: set WIRE_UI_SERVER_SRC, ALCHEMY_OBJECTS_SRC and OOREXX_CRYPTO_SRC"
fi
if [[ -n "${WIRE_UI_BUILDER_SRC:-}" && -n "${ALCHEMY_OBJECTS_SRC:-}" && -n "${OOREXX_CRYPTO_SRC:-}" ]]; then
  ...
  rexx "$ROOT/integration/test_builder_v083_contract.rex"
else
  echo "SKIP Builder v0.8.3 contract: set WIRE_UI_BUILDER_SRC, ALCHEMY_OBJECTS_SRC and OOREXX_CRYPTO_SRC"
fi
