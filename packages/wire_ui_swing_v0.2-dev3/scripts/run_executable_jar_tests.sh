...
JAR="$ROOT/build/wire-ui-swing-v0.2-dev3.jar"

VERSION_OUTPUT="$(java -jar "$JAR" --version)"
[[ "$VERSION_OUTPUT" == "wire-ui-swing 0.2-dev3" ]]
...
grep -Fq 'wire-ui-swing 0.2-dev3 READY view=SEARCH revision=0' <<<"$SMOKE_OUTPUT"
...
xvfb-run -a bash -c '
  ...
  for _ in $(seq 1 50); do
    if grep -Fq "wire-ui-swing 0.2-dev3 READY view=SEARCH revision=0" "$2" 2>/dev/null; then
