...
config = .GrokProviderConfig~new("http://127.0.0.1:" || port || "/v1/chat/completions", "provider.primary", models, 10, .true, 32)
secrets = .TestSecretProvider~new
secrets~put("provider.primary", secretMarker)
broker = .SecretBroker~new(secrets)
transport = .GrokCurlTransport~new("curl", .true)
