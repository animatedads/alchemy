...
main:
  ...
  call value "AI_GROK_CREDENTIAL_ENV", "AI_GROK_TEST_SECRET", "ENVIRONMENT"
  call value "AI_GROK_TEST_SECRET", secretMarker, "ENVIRONMENT"
  call value "AI_GROK_MODELS", "fixture-model", "ENVIRONMENT"
  call value "AI_GROK_TIMEOUT", "5", "ENVIRONMENT"
  call value "AI_GROK_MAX_OUTPUT", "32", "ENVIRONMENT"
  call value "AI_GROK_CURL", packageRoot || "/tests/fixtures/fake_curl.sh", "ENVIRONMENT"
  ...
  call ok kernel~activate("prod", "ai.provider.grok", module~generationId), "activate Grok provider"

  abilityRegistry = .AbilityRegistry~new(kernel)
  loaded = .AbilityProfileLoader~readFile(packageRoot || "/fixtures/profiles/grok.ability"); call ok loaded, "load Grok profile"
  stagedProfile = abilityRegistry~stage("prod", loaded~value); call ok stagedProfile, "stage Grok profile"; profileGeneration = stagedProfile~value
  call ok abilityRegistry~activate("prod", "client-ai", profileGeneration~generationId), "activate Grok profile"
  ...
  bucket = .WLUCapacityBucket~new("grok", 6000000, 0, clock~nowTick)
  ...
  modelPolicy = .GrokModelPolicy~new(.array~of("fixture-model"), 32)
  budgetPolicy = .GrokTokenBudgetPolicy~new(1, 8, 0, 30)
  planner = .GrokWLUPlanner~new(modelPolicy, .GrokConservativeTokenEstimator~new(budgetPolicy), budgetPolicy)
  bridge = .AbilityWLUBridge~new(authority)
  call ok bridge~registerPlanner("model.complete", planner), "register Grok WLU planner"
  ...
  call eq 200, status(response), "WLU-managed Grok completion"
  ...
  call eq "work.load.units/0.12", execution~at("wlu_api_version"), "WLU API version"
  ...
  hold = authority~reserve("client-ai", "ABILITY:MODEL.COMPLETE", 5500000, 30, "exhaust-grok")
  ...
  call ok server~stop, "stop Grok WLU server"
  call awaitStopped server
  call value "AI_GROK_TEST_SECRET", "", "ENVIRONMENT"
...
stageBundle:
  ...
  call ok builder~addFile(packageRoot || "/src/GrokProvider.cls", "GrokProvider.cls"), "add Grok provider"
  built = builder~build; call ok built, "build Grok provider bundle"
  bundle = built~value
  call ok verifier~pin("ability:ai:grok:v3", bundle~sourceLines), "pin Grok bundle"
  artifact = .RuntimeArtifact~new("ai.provider.grok", "CAPABILITY", "0.4.0", "ability:ai:grok:v3", "GrokRuntimeModule", bundle~sourceLines, .GrokProviderBuild~API_VERSION)
  staged = kernel~stage("prod", artifact); call ok staged, "stage Grok provider"
  return staged~value
