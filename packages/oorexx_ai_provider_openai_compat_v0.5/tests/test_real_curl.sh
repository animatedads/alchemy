...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/openai-compat-curl.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
PORTFILE="$TMP/port"
STATUSFILE="$TMP/status"
SECRET='FAKE-OPENAI-SECRET-7E4B'
"$REXX_BIN" "$ROOT/tests/openai_fixture_server.rex" "$PORTFILE" "$STATUSFILE" "$SECRET" >"$TMP/server.out" 2>"$TMP/server.err" &
server_pid=$!
