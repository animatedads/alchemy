...
"$REXX_BIN" "$ROOT/tests/test_openai_compat_provider.rex" "$ROOT"
"$REXX_BIN" "$ROOT/tests/test_openai_runtime_module.rex" "$ROOT"
...
if [[ -n "${RUNTIME_REGISTRY_ROOT:-}" || -n "${WLU_ROOT:-}" ]]; then
  ...
  "$REXX_BIN" "$ROOT/tests/test_openai_wlu_planner.rex" "$ROOT"
  "$REXX_BIN" "$ROOT/tests/test_openai_wlu_http.rex" "$ROOT"
else
  echo "SKIP OpenAI-compatible WLU qualification (RUNTIME_REGISTRY_ROOT/WLU_ROOT not set)"
fi
