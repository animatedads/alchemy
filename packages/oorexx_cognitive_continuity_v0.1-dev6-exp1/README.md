# ooRexx Cognitive Continuity v0.1-dev6-exp1
...
## dev6: retaining Qwen proposals without admitting them

`COGNITIVE.LEARNING.RESULT.RECORD` accepts the exact `cognitive.learning.request/2` together with the returned `cognitive.learning.result/2`. The service verifies request/scope correlation, rejects authority-bearing output, validates proposal `basisRefs` against evidence that was actually present in that request, and appends the result only to the measurement journal.

`COGNITIVE.LEARNING.RESULTS` lists those captured results. They are not cognitive records and are not automatically included in later learning requests; this intentionally prevents a model from recursively training on its own prior proposals without independent evidence.
