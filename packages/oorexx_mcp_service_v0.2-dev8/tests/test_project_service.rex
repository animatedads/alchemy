...
release=.directory~new; release['componentId']='wire_ui_server'; release['reason']='qualification release'
r=service~dispatch('PROJECT.OWNERSHIP.RELEASE',release,'llm:alice')
call assert \r~ok & r~code='OWNERSHIP_RELEASE_FORBIDDEN', 'non-owner cannot release ownership'
r=service~dispatch('PROJECT.OWNERSHIP.RELEASE',release,'llm:bob')
call assert r~ok & r~code='OWNERSHIP_RELEASED', 'active owner releases ownership'
call assert r~value['status']~string='RELEASED' & r~value['ownershipId']~string<>'', 'release evidence returned'
r=service~dispatch('PROJECT.OWNERSHIP.RELEASE',release,'llm:bob')
call assert r~ok & r~code='ALREADY_UNOWNED', 'release is idempotent once unowned'
r=service~dispatch('PROJECT.OWNERSHIP.CLAIM',claim,'llm:alice')
call assert r~ok & r~code='OWNERSHIP_CLAIMED', 'component can be re-claimed after release'
...
r=service2~dispatch('PROJECT.OWNERSHIP.CLAIM',claim,'llm:bob')
call assert \r~ok & r~code='OWNERSHIP_CONFLICT', 'journal replay preserves post-release re-claimed ownership'
r=service2~dispatch('PROJECT.OWNERSHIP.RELEASE',release,'llm:alice')
call assert r~ok & r~code='OWNERSHIP_RELEASED', 'replayed owner can release ownership'
...
/* Spheres are first-class ownable project components.  The canonical ID is
 * namespaced so a sphere can coexist with a runtime component of the same
 * logical name.  Unique legacy/filename aliases resolve to the canonical ID. */
call assert catalog~has('sphere:oorexx-llm-pitfalls'), 'sphere component exists in project catalog'
call assert catalog~has('oorexx_llm_pitfalls'), 'sphere filename alias resolves'
sphereRow=catalog~get('oorexx_llm_pitfalls')
call assert sphereRow<>.nil & sphereRow['componentKind']~string='sphere', 'sphere catalog row is typed'
call assert sphereRow['componentId']~string='sphere:oorexx-llm-pitfalls', 'sphere alias canonicalizes to namespaced component id'
call assert sphereRow['sphereId']~string='oorexx-llm-pitfalls', 'sphere component preserves Gopher sphere id'

sphereClaim=.directory~new; sphereClaim['componentId']='oorexx_llm_pitfalls'
r=service~dispatch('PROJECT.OWNERSHIP.CLAIM',sphereClaim,'llm:claude')
call assert r~ok & r~code='OWNERSHIP_CLAIMED', 'sphere ownership claim succeeds via alias'
call assert r~value['componentId']~string='sphere:oorexx-llm-pitfalls', 'sphere ownership journal uses canonical id'

sphereMail=.directory~new; sphereMail['componentId']='sphere:oorexx-llm-pitfalls'; sphereMail['body']='Maintain stale-model/access-point guidance.'
r=service~dispatch('PROJECT.REQUEST.CREATE',sphereMail,'architect')
call assert r~ok, 'request can target sphere component'
call assert r~value['recipientId']~string='llm:claude', 'sphere request routes to active sphere owner'
sphereReq=r~value['requestId']~string

sphereList=.directory~new; sphereList['componentId']='oorexx_llm_pitfalls'; sphereList['unreadOnly']=.true
r=service~dispatch('PROJECT.REQUESTS.LIST',sphereList,'llm:claude')
call assert r~ok & r~value['count']=1, 'sphere owner sees sphere request through alias filter'

sphereRelease=.directory~new; sphereRelease['componentId']='oorexx_llm_pitfalls'; sphereRelease['reason']='sphere component qualification complete'
r=service~dispatch('PROJECT.OWNERSHIP.RELEASE',sphereRelease,'llm:claude')
call assert r~ok & r~code='OWNERSHIP_RELEASED', 'sphere ownership releases independently of parent/runtime components'

/* A colliding logical sphere name must not shadow an exact runtime component. */
call assert catalog~get('civicport')['componentKind']~string='runtime', 'exact runtime component wins over sphere convenience alias'
call assert catalog~get('sphere:civicport')['componentKind']~string='sphere', 'namespaced sphere remains independently addressable'
