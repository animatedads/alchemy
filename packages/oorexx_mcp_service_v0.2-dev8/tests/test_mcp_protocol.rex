...
r=protocol~handle(headers,.json~toJSON(req),'')
d=.json~fromJSON(r~jsonText)
call assert d['result']['tools']~items=8,'eight tools listed'
...
/* Release ownership through MCP after an authenticated claim. */
headers['mcp-name']='project.ownership.claim'; params['name']='project.ownership.claim'; args=.directory~new; args['componentId']='oorexx_https_server'; params['arguments']=args
req['id']=5
r=protocol~handle(headers,.json~toJSON(req),'llm:https')
d=.json~fromJSON(r~jsonText)
call assert d['result']['isError']=.false & d['result']['structuredContent']['code']~string='OWNERSHIP_CLAIMED','authenticated claim succeeds'
headers['mcp-name']='project.ownership.release'; params['name']='project.ownership.release'; args=.directory~new; args['componentId']='oorexx_https_server'; args['reason']='protocol qualification'; params['arguments']=args
req['id']=6
r=protocol~handle(headers,.json~toJSON(req),'llm:https')
d=.json~fromJSON(r~jsonText)
call assert d['result']['isError']=.false & d['result']['structuredContent']['code']~string='OWNERSHIP_RELEASED','authenticated release succeeds'
