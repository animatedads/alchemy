...
if port = "" then port = 1636

dir = .IdentityDirectory~new("ldaps-wire-fixture")
ignore = dir~createPrincipal("principal:alice", "uid=alice,ou=people,dc=example,dc=org", "PERSON", "Alice LDAPS", "alice")
...
server = .LdapWireServer~new(service, "127.0.0.1", port, "dc=example,dc=org", tlsProvider, .nil, .true)
served = server~serve(2, readyFile)
tlsProvider~close
say "LDAP LDAPS FIXTURE DONE connections=" || served
