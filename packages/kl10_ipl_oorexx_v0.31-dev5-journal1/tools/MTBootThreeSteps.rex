/* MTBootThreeSteps.rex
 * Establish real MTBOOT.EXB state, execute exactly TDZ, SKIPA and the bounded
 * CONO APR,200000 reset transition, halting after each.  Show 040004 only.
 */
numeric digits 30
parse arg tapePath
if tapePath = "" then do
  say "usage: rexx tools/MTBootThreeSteps.rex <bb-h137f-bm.tap>"
  exit 2
end
...
say "APR reset count:" cpu~apr~ioResetCount
...
do stepNo = 1 to 3
  ...
  if trace["ioAction"] \= .nil then say "  I/O action:" trace["ioAction"] "APR reset count=" cpu~apr~ioResetCount
end
