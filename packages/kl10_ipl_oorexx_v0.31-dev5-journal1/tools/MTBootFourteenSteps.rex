/* Demonstrate exactly fourteen bounded instructions from the real MTBOOT.EXB.
 * The final three are MOVEI/MOVEM/MOVEI; then show the next LSH without
 * executing it.
 */
numeric digits 30
parse arg tapePath
if tapePath = "" then do
  say "usage: rexx MTBootFourteenSteps.rex /path/to/bb-h137f-bm.tap"
  exit 2
end
...
do n = 1 to 14
  ...
  if t["mnemonic"] = "MOVEI" then
    say "  E:" oct18(t["effectiveAddress"]) "AC" oct2(t["ac"]) "=" oct36(t["acAfter"])
  if t["mnemonic"] = "MOVEM" then
    say "  E:" oct18(t["effectiveAddress"]) "stored:" oct36(t["result"])
end
