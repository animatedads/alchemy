...
Console facade v0.28:
- No new guest architecture is inferred by the keyboard facade. Host key bytes enter only through the existing `KL10Dte~type()` RX queue and are delivered by the already-tested DTE20 secondary service path.
- The supplied `mtboot_prompt_v0.27(1).kl10state` is used as acceptance evidence for the first real monitor-input wait and for the post-ENTER `IDPB` execution boundary.
