/* Real MTBOOT.EXB: execute exactly eight admitted instructions.  The eighth
 * is JSP 16,17 at 040010, which saves 000000,,040011 in AC16 and transfers
 * PC to 000017.  Fetch/decode the resulting AC17 instruction but do not
 * execute it.
 * Usage: rexx test_mtboot_eight_steps.rex /path/to/bb-h137f-bm.tap
 */
numeric digits 30
parse arg tapePath
if tapePath = "" then do
  say "usage: rexx test_mtboot_eight_steps.rex /path/to/bb-h137f-bm.tap"
  exit 2
end
...
/* PC=17 octal is the AC window.  Fetch the next instruction from AC17 and
 * decode it, but do not execute JRST yet. */
nextWord = cpu~fetch
next = cpu~nextInstruction
call assertEq nextWord, octToDec("254016000000"), "word fetched from AC17"
call assertEq cpu~pc, octToDec("17"), "AC17 instruction remains unexecuted"
...
call assertEq next["ac"], 0, "AC17 JRST AC field"
...
tape~close
say "KL10 MTBOOT eight-step acceptance: PASS"
...
say "  next fetched from AC17, not executed: 254016,,000000  JRST 0(16)"
exit 0
