/* Real MTBOOT.EXB: execute exactly six admitted instructions, stopping
 * after indexed CONO PAG,0(15) at 040006, then expose 040007 without
 * executing it.
 * Usage: rexx test_mtboot_six_steps.rex /path/to/bb-h137f-bm.tap
 */
numeric digits 30
parse arg tapePath
if tapePath = "" then do
  say "usage: rexx test_mtboot_six_steps.rex /path/to/bb-h137f-bm.tap"
  exit 2
end
...
sixthIns = cpu~nextInstruction
call assertEq sixthIns["format"], "IO", "sixth format"
call assertEq sixthIns["mnemonic"], "CONO", "sixth mnemonic"
call assertEq sixthIns["deviceName"], "PAG", "sixth device"
call assertEq sixthIns["index"], ac15, "sixth X=15 octal"
call assertEq sixthIns["address"], 0, "sixth Y=0"

sixth = cpu~step
call assertEq sixth["ioAction"], "PAG_CONO_ZERO", "sixth action"
call assertEq sixth["ioIndexRightHalf"], 0, "real AC15 right half"
call assertEq sixth["effectiveAddress"], 0, "real indexed E"
call assertEq sixth["pagStatusAfter"], 0, "PAG status after CONO zero"
call assertEq sixth["pagConoZeroCountAfter"], 1, "PAG CONO evidence count"
call assertEq sixth["pcAfter"], octToDec("040007"), "indexed PAG CONO next PC"
call assertEq cpu~halted, 1, "halt after sixth step"

/* Decode the next real word, but do not execute it. */
nextWord = cpu~fetch
next = cpu~nextInstruction
call assertEq nextWord, octToDec("205740254016"), "unexecuted word at 040007"
call assertEq cpu~pc, octToDec("040007"), "040007 remains unexecuted"
call assertEq next["format"], "GENERAL", "040007 format"
call assertEq next["mnemonic"], "MOVSI", "040007 mnemonic"
call assertEq next["opcode"], octToDec("205"), "040007 opcode"
call assertEq next["ac"], octToDec("17"), "040007 AC17"
call assertEq next["indirect"], 0, "040007 indirect"
call assertEq next["index"], 0, "040007 index"
call assertEq next["address"], octToDec("254016"), "040007 address/immediate field"

tape~close
say "KL10 MTBOOT six-step acceptance: PASS"
...
say "  next, not executed: 040007  205740,,254016  MOVSI 17,254016"
exit 0
