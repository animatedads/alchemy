...
## dev4 — Storage sequential-media provenance

dev4 adds `ProvenanceSequentialMediaAdapter`, an optional duck-typed seam for Storage Fabric v0.1-dev17 sequential media. It commits canonical CARD_DECK/PAPER_LISTING/TAPE_VOLUME shape, explicit edge-representation evidence (for example ASCII reader decks, punch output and AWSTAPE), and round-trip verification events without storing media bytes or redefining Storage authority. Representation events distinguish canonical object state from derived edge artefacts and can record a loss/reconstruction profile such as Hercules ASCII punch trailing-blank behaviour.