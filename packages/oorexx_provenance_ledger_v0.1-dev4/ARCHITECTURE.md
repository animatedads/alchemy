...
## Sequential media provenance (dev4)

Storage Fabric remains authoritative for card, paper and tape record semantics. Provenance Ledger records commitments only. `ProvenanceSequentialMediaAdapter` therefore binds a stable Storage object/digest to the canonical media shape and, separately, to explicitly named edge representations. An AWSTAPE file, Hercules ASCII reader deck, punch file or printer-text projection is evidence about a representation; it does not silently become the canonical object. Import/export and round-trip events preserve that distinction, including declared lossy/reconstructive edge behaviour.