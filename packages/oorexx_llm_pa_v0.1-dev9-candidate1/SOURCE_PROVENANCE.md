...
## dev9-candidate1 machine-ability integration

This candidate is based on the user-supplied mid-session Codex WIP archive `oorexx_llm_pa_v0.1-dev8(3).zip`, not on the previously sealed dev8 artifact.  It adds the SecurityManager-contained read-only machine-observation surface and does not claim to seal or supersede Codex's concurrent WIP until target-host qualification is accepted.
