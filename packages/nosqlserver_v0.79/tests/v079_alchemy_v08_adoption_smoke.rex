/* NoSQLServer v0.79 Alchemy Objects v0.8 STANDARD adoption / INIT construction smoke. */
call directory directory("S")

root = .NoSQLServerTestSupport~createBlankDatabase("v079-alchemy")
...
call assert .AlchemyAdoptionVerifier~verify(file, "STANDARD")~ok, "file engine satisfies v0.8 STANDARD adoption"
fileVerification = .AlchemyAdoptionVerifier~verify(file, "STANDARD")
call assert fileVerification~evidence["base_version"] = "0.8", "verifier observes Alchemy Objects v0.8"
call assert fileVerification~evidence["construction_provenance"]["entrypoint"] = "INIT", "file engine uses preferred INIT:SUPER construction"
checkpoint = .AlchemyAdoptionVerifier~checkpoint(file, "STANDARD")
call assert .AlchemyAdoptionVerifier~compareCheckpoint(file, checkpoint)~ok, "STANDARD adoption checkpoint round-trips"
...
call assert v~supports("ALCHEMY_OBJECT_BASE_0_8"), "v0.8 house-base capability advertised"
call assert v~supports("ALCHEMY_OBJECT_STANDARD_0_8"), "v0.8 STANDARD adoption capability advertised"
call assert v~supports("ALCHEMY_OBJECT_INIT_SUPER_CONSTRUCTION"), "preferred INIT:SUPER construction advertised"
call assert v~supports("ALCHEMY_OBJECT_ADOPTION_CHECKPOINT"), "adoption checkpoint capability advertised"
call assert v~supports("ALCHEMY_OBJECT_EXECUTION_PROVENANCE"), "execution provenance capability advertised"
...
say "NOSQLSERVER V0.79 ALCHEMY V0.8 STANDARD / INIT ADOPTION: OK"
exit 0
