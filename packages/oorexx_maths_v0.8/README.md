...
Assistant-host v0.8 qualification: **343/343 executed assertions PASS** when the 6 Crypto sealing assertions are run through the qualified Foreign Runtime/OpenSSL Crypto provider. This comprises the prior 263 non-Crypto v0.7 assertions, **74 new Math3D assertions**, and 6 Crypto sealing assertions. FLINT-ARB remains an 8-assertion capability-gated lane to rerun on the user's python-flint-enabled host; if unchanged it raises the cross-environment total to **351/351**.
