# Object Queue Fabric v0.9-dev6 (aggregate API 0.9; broker API 0.2)
...
## v0.9-dev6 secure inter-node transport slice
...
The v0.9-dev6 transport also bounds TCP connection establishment per endpoint. Primary and failover endpoint registration accept an optional `connectTimeoutMs` (default 5000 ms). The connector performs non-blocking connect plus `select`/`SO_ERROR`, then restores blocking mode before the existing authenticated HELLO/CHALLENGE exchange. A silently dropped SYN therefore returns transport failure within the configured bound instead of stalling the calling queue pump for the operating system TCP timeout.

The v0.9-dev6 TCP transport is `queue.transport/2`. It requires an explicit exact source-IP allowlist for each trusted peer and rejects an unlisted accepted socket before HELLO processing. After the authenticated challenge, DELIVER/PROBE/RESULT/HEALTH bodies are ChaCha20-encrypted and authenticated with a separate HMAC-SHA-512 key derived per connection from the transport PSK, authenticated transcript and fresh client/server nonces. HELLO/CHALLENGE metadata remains authenticated but visible. This PSK transport does **not** claim forward secrecy; see `SECURE_TRANSPORT.md` for the security and OCI deployment boundary.
