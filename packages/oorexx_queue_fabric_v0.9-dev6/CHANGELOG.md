...
- Keeps the encrypted `queue.transport/2` wire protocol and durable record identities unchanged.
- Bounds outbound TCP connection establishment with a non-blocking `FIONBIO` + `select` + `SO_ERROR` connector so silently dropped SYN packets cannot stall queue delivery indefinitely.
- Adds per-endpoint `connectTimeoutMs` (default 5000 ms) to primary and failover endpoint registration while preserving existing call signatures.
- Restores the connected socket to blocking mode before the existing authenticated HELLO/CHALLENGE and encrypted DELIVER/PROBE exchange.
- Reports `CONNECT_TIMEOUT`, `CONNECT_FAILED`, or connector-preflight detail beneath the existing `REMOTE_MANAGER_UNAVAILABLE` transport result so failover behavior remains compatible.
