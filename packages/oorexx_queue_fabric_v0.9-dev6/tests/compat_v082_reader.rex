/* Exact v0.8.2 reader for v0.9-created durable state. */
parse arg root
if root = "" then exit 2
...
call assert package~priority = 8, "priority recovered"
call assert package~payload~isA(.Directory), "payload remains Directory"
call assert package~payload["kind"] = "V09", "top-level value recovered"
...
call assert package~payload["nested"][2]["value"] = 90, "nested value recovered"
say "OBJECT QUEUE FABRIC V0.9 -> V0.8.2 REPLAY: OK"
exit 0
