/* Strong worker-side checker for dev7 managed initial placement. */
...
if st~get('migratable_job_version','')<>.MigratableJobBuild~VERSION | .MigratableJobBuild~VERSION<>'0.2.5' then do; say 'FD_MIGRATABLE_CHECK_ERROR Migratable Job baseline='||st~get('migratable_job_version',''); exit 1; end
