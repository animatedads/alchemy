...
for t in \
  ...
  test_video_hardening.rex \
  test_temporal_deflection.rex \
  test_source_stability_guard.rex \
  test_source_mutation_abort.rex \
  test_worker_posix_guard.rex \
...
if [ "${FD_TEST_MIGRATABLE_STARTER:-0}" = "1" ]; then
  ...
  if [ "${FD_TEST_REMOTE_START:-0}" = "1" ]; then
    echo "== test_remote_start_fd_v025.rex =="
    "$REXX" "$ROOT/tests/test_remote_start_fd_v025.rex" "$ROOT"
  fi
fi
