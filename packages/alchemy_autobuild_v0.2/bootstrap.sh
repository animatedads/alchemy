...
if [ ! -f "$ROOT/config.json" ]; then
  cp "$REPO/packages/alchemy_autobuild_v0.2/config.example.json" "$ROOT/config.json"
fi
...
[Service]
Type=simple
ExecStart=/usr/bin/python3 $REPO/packages/alchemy_autobuild_v0.2/alchemy_autobuild.py --config $ROOT/config.json
Restart=on-failure
RestartSec=2
