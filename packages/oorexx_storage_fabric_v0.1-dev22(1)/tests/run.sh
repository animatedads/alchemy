...
cd "$(dirname "$0")/.."
native/build-host-filesystem.sh
...
for t in \
  ...
  tests/test_host_filesystem_provider.rex \
  ...
  tests/test_large_file_precision.rex \
...
tests/test_host_backup_floor_cli.sh
...
if [[ -n "${STORAGE_SFTP_SRC:-}" && -n "${STORAGE_SSH_SRC:-}" && -n "${FOREIGN_RUNTIME_SRC:-}" ]]; then
  REXX_PATH="$PWD/src:${STORAGE_SFTP_SRC}:${STORAGE_SSH_SRC}:${FOREIGN_RUNTIME_SRC}${REXX_PATH:+:${REXX_PATH}}" "$REXX_BIN" tests/test_sftp_backend.rex
  echo "PASS optional Storage <-> SFTP backend integration"
fi
echo "PASS ALL STORAGE FABRIC v0.1-dev21 CORE TESTS"
