# ooRexx Storage Fabric v0.1-dev22
...
## New in v0.1-dev22 — canonical durable checkpoint integers

A live backup exposed a second manifestation of caller-context numeric formatting in dev20 checkpoint evidence: the final Adler-32 field was persisted as scientific notation (for example `2.18468359E+9`).  That field is a durable integrity checksum, not display text.

dev22 makes checkpoint numeric serialization explicit rather than relying on Rexx numeric-to-string formatting.  Sequence, total bytes, progress bytes, chunk bytes and the Adler-32 trailer are emitted as canonical non-negative decimal integers: digits only, no exponent notation, whitespace, signs or leading zeroes.  The checkpoint reader validates this form before converting the value and rejects legacy/scientific numeric evidence fail-closed.

This is deliberately stricter than merely running the package under `DIGITS 30`: durable wire/disk formats must not depend on caller or runtime rendering policy.

`tests/test_large_file_precision.rex` now checks both the exact 2,005,735,208-byte arithmetic regression and the checkpoint text itself, including rejection of a synthetic `2.18468359E+9` checksum trailer.

## New in v0.1-dev21 — exact large-file byte arithmetic hotfix

Production backup of a 2,005,735,208-byte AVI exposed a real >9-digit precision defect in the local streaming path. ooRexx defaults to `NUMERIC DIGITS 9`; dev20 converted file sizes/offsets with `+0` inside packages that did not establish a wider arithmetic context. The resulting rounded logical size could make the final read request extend past the real EOF and surface as `Error 88.900: short source read`.

dev21 establishes `::OPTIONS DIGITS 30` in the Storage core, streaming and host-filesystem packages so byte counts, offsets, checkpoint progress and host source fences remain exact for ordinary 64-bit file sizes regardless of the caller's numeric context. `StorageLocalFileByteSource~readChunk(maximum)` is also corrected to honour its at-most contract: a positive short `CHARIN` is returned as a valid chunk and only zero progress before the fenced source size is treated as premature EOF/source change.

`tests/test_large_file_precision.rex` is a sparse-file regression at the exact production size `2005735208`; it runs the caller at `NUMERIC DIGITS 9`, reads the final eight bytes through Storage, and checks exact transfer/checkpoint/host-fingerprint counters.

No destination commit, checkpoint durability, source read-only, verification or no-replace publication rule is weakened by this fix.

## New in v0.1-dev20 — backup-task handoff and filesystem-boundary safety

- retains `storage.fabric.host-filesystem/0.1` and adds the operator/task handoff `bin/storage-host-backup-floor`;
- `preflight` verifies source/destination policy, distinct filesystem identity, destination Linux `rw`, creates the destination-resident checkpoint directory and executes a real no-replace/fsync publication probe before user data is admitted;
- `copy` exposes exactly one resumable verified regular-file commit; `verify` independently re-hashes an already committed source/destination pair;
- source mount identity now records the root filesystem device. `byteSource` refuses regular-file bytes crossing that device boundary, so a `/` source mount cannot silently consume `/proc`, `/sys`, another mounted disk or the USB backup target; the audit/orchestration task may mount each intended filesystem separately;
- the native commit helper now prefers Linux `renameat2(RENAME_NOREPLACE)` and falls back to the dev19 hard-link publication route, extending safe no-replace commit to filesystems that do not support hard links when the kernel/filesystem provides atomic no-replace rename;
- idempotent reconciliation re-syncs an already verified final file and its parent directory before reporting `ALREADY_COMMITTED`;
- adds a self-cleaning destination commit-filesystem probe and a tab-framed/hex-valued `SFHOST1` machine surface for the prioritised backup task;
- `StorageHostFilesystemMount` binds an existing Linux directory as either `READ_ONLY` or `WRITE_COMMIT` Storage capacity without making the host pathname Storage object identity;
- source mounts expose metadata/enumeration and bounded regular-file reads only; symlinks are observed as symlinks and are not silently followed as file content;
- destination paths are Storage-relative, reject traversal, and refuse symlink directory components / partial-file / final-file escapes;
- `StorageHostAtomicFileSink` writes to a deterministic hidden partial and uses the existing two-slot Storage transfer checkpoint for restart/resume;
- every durable checkpoint follows stream flush plus native `fdatasync()` of the partial file;
- `StorageHostVerifiedCopy` performs bounded COPY -> independent SHA-256 VERIFY -> source re-probe -> no-replace COMMIT;
- `native/storage_host_commit.c` publishes with atomic no-replace rename where supported and a no-replace hard-link fallback, then syncs the file and containing directory;
- committed final names are never overwritten in place: same verified bytes reconcile idempotently, different bytes at the same final path fail closed;
- Linux mount options are observable separately (`kernelReadOnly` / `kernelReadWrite`) so rescue/runlevel-1 backup policy can require an actually read-only source filesystem where practical;
- this is deliberately the mechanical floor only: backup ordering, retention, pseudo-filesystem exclusion, metadata policy and source deletion decisions remain above Storage Fabric.

See `HOST_FILESYSTEM.md`.

## New in v0.1-dev18 — SSH/SFTP namespace endpoint

- adds optional API `storage.fabric.sftp/0.1` in `src/StorageSftp.cls`;
- `StorageSftpBackend` implements the existing ooRexx `SftpBackend` contract by delegating path/object operations to `StorageFuseOperationCore`;
- SFTP paths are virtual Storage namespace paths under an explicitly configured Storage root, never host filesystem locators;
- read-only is the default adapter policy; mutation must be enabled explicitly and still remains subject to Storage FUSE semantics;
- SFTP v3 mkdir/open/read/write/append/readdir/rename/remove/rmdir have executable integration coverage; non-empty `rmdir` fails closed and no recursive delete is inferred;
- `StorageSftpBackendFactory~subsystemHandler` plugs directly into `SshEndpointRegistry` as subsystem `sftp`; SSH authentication/endpoint authority remains outside Storage;
- ordinary empty live-directory removal is now supported by the Storage FUSE semantic core so protocol adapters need not bypass Storage state;
- incorporates the 2026-09-19 FUSE/RPC maintainability review: method-level comments and exact `TRUNCATEH` RPC arity;
- **SCP is not claimed**: the supplied SSH v0.1-dev1 package explicitly leaves SCP to a sibling protocol consumer. A future SCP endpoint should consume the same Storage authority seam rather than invoking a shell or turning remote paths into Storage identity.
