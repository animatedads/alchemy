...
## SSH/SFTP integration (dev18)

Optional dependency closure for the Storage-backed SFTP endpoint:

- ooRexx SSH v0.1-dev1 (`ssh/0.1`);
- ooRexx SFTP v0.1-dev1 (`sftp/0.1`);
- Foreign Runtime v0.22.6 through the SSH package;
- Storage Fabric v0.1-dev18 (`storage.fabric.sftp/0.1`).
...
The final `.false` keeps the exported Storage namespace read-only. Set mutation authority explicitly only where the embedding application intends SFTP writes. SSH endpoint authority remains a separate decision.

No SCP implementation is bundled or claimed in this release.


## Host filesystem / backup task boundary (dev20)

The backup task may mount the machine source through `StorageHostFilesystemMount(..., "READ_ONLY")` and a USB/durable target through `StorageHostFilesystemMount(..., "WRITE_COMMIT")`.  It owns traversal order and priority.  For each selected regular file it may call `StorageHostVerifiedCopy~copyFile`, passing a stable transfer id and checkpoint root.

The task must not interpret a `PAUSED` result as committed.  Only `COMMITTED` or `ALREADY_COMMITTED` with a non-empty SHA-256 verification reference is a published regular-file result.  A changed source, digest mismatch, path collision or different already-committed final object fails closed.

`READ_ONLY` is a Storage policy fact; `kernelReadOnly` is an independent Linux observation.  A runlevel-1/rescue workflow can require both for the source when the root filesystem can be remounted `ro`.  The destination should report `kernelReadWrite` and be explicitly selected by the task; Storage does not auto-discover a USB volume and start writing to it.


### Backup task CLI handoff

The audit/prioritisation task can consume `bin/storage-host-backup-floor` or call
`StorageHostBackupPreflight` / `StorageHostVerifiedCopy` directly. The CLI uses
`SFHOST1` tab framing with hex values. It does not enumerate recursively or
choose exclusions; it refuses cross-filesystem source byte reads and keeps
resume checkpoints under the destination by default.


## Large-file backup precision (dev21)

Backup/task callers may run with the ordinary ooRexx numeric context. Storage owns exact byte arithmetic internally. A production 2,005,735,208-byte AVI exposed the previous dev20 rounding defect; dev21 widens the relevant package arithmetic context and adds an exact sparse-file regression at that size. No caller-side `NUMERIC DIGITS` workaround is required.
