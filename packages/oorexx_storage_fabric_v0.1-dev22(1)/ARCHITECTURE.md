...
## SSH/SFTP projection boundary (dev18)

`StorageSftpBackend` is an optional compatibility adapter.  The authority chain is:

```text
SSH transport/authentication -> SFTP v3 endpoint -> StorageSftpBackend -> StorageFuseOperationCore -> Storage namespace/state
```

SFTP owns packet/handle semantics. SSH owns encrypted transport, identity attribution and endpoint dispatch. Storage owns namespace identity, generations, streams and mutation semantics. The adapter never substitutes a host pathname for a `StorageRef` or namespace path, and it does not create an implicit shell/exec path. Read-only is the default policy.

The current SSH/SFTP dev1 bundle does not implement SCP. Storage therefore does not emulate SCP using a shell command. When an SCP protocol endpoint exists, it should terminate on the same Storage semantic authority rather than introducing a second remote-file model.


## Host-filesystem backup floor (dev20)

`StorageHostFilesystemMount` is a provider seam, not a backup policy engine.  A Linux source tree is bound `READ_ONLY`; a destination filesystem such as a USB3 mount is bound `WRITE_COMMIT`.  The source and destination remain asymmetric even when both happen to be ordinary POSIX filesystems.

Regular-file movement reuses `StorageResumableTransferEngine`.  Bytes first land in a hidden deterministic partial.  The existing transfer checkpoint is advanced only after the sink has flushed and the native helper has `fdatasync()`ed that partial.  SHA-256 verification remains distinct from copy completion.  Publication is distinct again: the helper creates the final name with a no-replace hard link, syncs the published file and parent directory, removes the partial, and syncs the directory again.  `StorageTransfer~commit` occurs only after that publication succeeds.

The mount performs path confinement before byte access.  Source regular files must canonicalise under the configured root.  Symlinks are metadata and are not treated as regular-file sources.  Destination directory components must be real directories, and existing partial/final entries must be real regular files within the destination root.  This does not turn pathname checking into a hostile-user sandbox; qualification assumes the intended quiet rescue/runlevel-1 backup environment while still failing closed on accidental path escape.

Backup priority, inclusion/exclusion, retention, metadata reconstruction and deletion are explicitly outside this API.  Storage Evacuation or a dedicated backup task may consume this floor without moving those authorities into Storage Fabric.


### Backup-task command boundary (dev20)

The prioritised backup task owns selection/order. Storage supplies only
`preflight`, one-file `copy`, and `verify` mechanics through
`bin/storage-host-backup-floor`. Source regular-file reads are fenced to the
source mount's root filesystem device; crossing a Linux mount boundary requires
a separately admitted Storage source mount. Destination checkpoints live on the
WRITE_COMMIT filesystem by default, and preflight exercises the real no-replace
publication/fsync primitive before accepting the disk.


## Exact large-file byte arithmetic (dev21)

Storage byte counts and offsets are semantic integers, not caller-context numerics. The Storage core, streaming and host-filesystem packages therefore establish `::OPTIONS DIGITS 30`. This prevents the default ooRexx 9-digit arithmetic context from rounding multi-gigabyte sizes/checkpoint offsets. Local `readChunk(maximum)` is explicitly an at-most read contract; any positive short read advances by the bytes actually returned, while zero progress before the fenced source size fails closed.


### Canonical integer persistence (dev22)

High arithmetic precision and durable serialization are separate contracts.  Transfer checkpoint integers are encoded by `StorageCodec~unsignedText` and parsed only after `isCanonicalUnsigned` validation.  This prevents exponent notation or caller-precision numeric rendering from entering recovery evidence.  Legacy/scientific checkpoint numeric fields fail closed and cause a fresh transfer rather than an ambiguous resume.
