 ...
 * v0.2-dev5 deliberately contains no operator-supplied placement id, ownership
 * epoch or FD-specific authority receipt.  Those are obtained later through
 * Migratable Job v0.2.4 migratable.job.placement/1 and Job-to-Node v0.6.
 *
...
if value('FD_SOURCE_PLACEMENT_ID',,'ENVIRONMENT')<>'' | value('FD_SOURCE_OWNERSHIP_EPOCH',,'ENVIRONMENT')<>'' | value('FD_SOURCE_AUTHORITY_REF',,'ENVIRONMENT')<>'' then do
  say 'FD_LAUNCH_SPEC_ERROR dev5 forbids operator-supplied placement/ownership/authority fields; use migratable.job.placement/1'; exit 65
end
