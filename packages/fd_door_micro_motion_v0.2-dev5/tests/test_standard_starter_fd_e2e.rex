...
if startStatus~get('migratable_job_version','')<>'0.2.4' then call fail 'Migratable Job version'
...
if run['status']<>'OK' | run['version']<>'0.2-dev5' then call fail 'run status/version'
if run['migration_api']<>'migratable.job/0.2' then call fail 'run migration api'
if run['decoded_frames']+0<>75 | run['analysed_frames']+0<>75 | run['samples_emitted']+0<>69 then call fail 'unexpected tiny-video evidence counts'
if run['audio_used']<>'NO' then call fail 'audio boundary'
if \audit~verify then call fail 'placement audit integrity'
say 'PASS FD real tiny-video managed NEW start v0.2.4 frames=75 samples=69 placement='||lease~placementId||' epoch='||lease~ownershipEpoch
exit 0
