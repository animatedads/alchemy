...
/* Explicit AWSTAPE representation: DAV bytes are a view, Storage remains TAPE_VOLUME. */
tape=.StorageTapeVolumeCodec~newVolume('HTTP-TAPE')
.StorageTapeVolumeCodec~addBlock(tape,'AAA')
.StorageTapeVolumeCodec~filemark(tape)
.StorageTapeVolumeCodec~addBlock(tape,'BBBB')
aws=.WebDavAwsTapeCodec~render(tape)
headers=.directory~new; headers['content-type']=.WebDavTapeFormat~AWSTAPE_TYPE
request=.HttpRequest~new('PUT','/dav/docs/work.aws','/dav/docs/work.aws','', 'HTTP/1.1',headers,aws,.true)
r=adapter~beforeRequest(request,.nil); call eq r~status,201,'AWSTAPE PUT status'
o=storage~objectAt('/docs/work.aws'); call eq o~eaValue('storage.media.family',''),.StorageSequentialMediaFamily~TAPE_VOLUME,'AWSTAPE canonical family'
image=storage~representations~imageFor(o); call eq image~boundaryCount,1,'AWSTAPE filemark retained'
request=.HttpRequest~new('GET','/dav/docs/work.aws','/dav/docs/work.aws','', 'HTTP/1.1',.directory~new,'',.true)
r=adapter~beforeRequest(request,.nil); call eq r~status,406,'generic tape GET refused'
headers=.directory~new; headers['accept']=.WebDavTapeFormat~AWSTAPE_TYPE
request=.HttpRequest~new('GET','/dav/docs/work.aws','/dav/docs/work.aws','', 'HTTP/1.1',headers,'',.true)
r=adapter~beforeRequest(request,.nil); call eq r~status,200,'AWSTAPE GET status'; call eq r~body,aws,'AWSTAPE round trip body'
