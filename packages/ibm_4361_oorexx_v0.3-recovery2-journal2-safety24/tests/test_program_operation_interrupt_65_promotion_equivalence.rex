/* Safety21 exact X'65' microstate: live candidate -> rewind -> remove -> permanent. */
numeric digits 30
storage=.IBM370JournaledStorage~new(1048576,2048,'S21-65-PROMO-RAM'); m=.IBM4361Machine~new(1048576,storage); m~powerOn
...
s=.IBM4361JournalSession~new(m); s~beginInstruction; m~executor~installLiveMethod('architecturalOperationException',classifier); m~executor~installLiveMethod('programInterruptOperation',interrupt); call eq m~tick,'OK','live 65 executes'; livePsw=m~cpu~psw~rawHex; liveOld=m~storage~fetchHex(x2d('28'),8); liveIc=m~cpu~instructionCount; s~abortInstruction
...
m~executor~removeLiveMethod('architecturalOperationException'); m~executor~removeLiveMethod('programInterruptOperation'); call eq m~executor~hasMethod('OP65'),0,'no OP65'
s~beginInstruction; call eq m~tick,'OK','permanent 65 executes'; permPsw=m~cpu~psw~rawHex; permOld=m~storage~fetchHex(x2d('28'),8); permIc=m~cpu~instructionCount; s~commitInstruction
call eq permPsw,livePsw,'live/permanent PSW'; call eq permOld,liveOld,'live/permanent old'; call eq permIc,liveIc,'live/permanent IC'; call eq permOld,'00040001A000006A','exact old'; call eq permPsw,'00040000000002CA','exact new'
say 'PASS test_program_operation_interrupt_65_promotion_equivalence'; exit 0
