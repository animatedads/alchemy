/* Safety16 exact FF microstate: live classifier -> rewind -> remove -> permanent. */
numeric digits 30
storage=.IBM370JournaledStorage~new(1048576,2048,'S16-FF-PROMO-RAM')
...
m~storage~storeHex(x2d('20'),'FF04000D0001'); m~storage~storeHex(x2d('28'),'0004000160000020'); m~storage~storeHex(x2d('68'),'00040000000002CA')
...
m~executor~installLiveMethod('architecturalOperationException',classifier); m~executor~installLiveMethod('programInterruptOperation',interrupt)
call eq m~tick,'OK','live FF executes'
livePsw=m~cpu~psw~rawHex; liveOld=m~storage~fetchHex(x2d('28'),8); liveIc=m~cpu~instructionCount
s~abortInstruction
call eq m~cpu~psw~rawHex,prePsw,'rewind PSW'; call eq m~cpu~instructionCount,preIc,'rewind IC'; call eq m~storage~fetchHex(x2d('28'),8),preOld,'rewind old PSW'
m~executor~removeLiveMethod('architecturalOperationException'); m~executor~removeLiveMethod('programInterruptOperation')
call eq m~executor~hasMethod('OPFF'),0,'no OPFF after removal'
s~beginInstruction; call eq m~tick,'OK','permanent FF executes'; permPsw=m~cpu~psw~rawHex; permOld=m~storage~fetchHex(x2d('28'),8); permIc=m~cpu~instructionCount; s~commitInstruction
call eq permPsw,livePsw,'live/permanent PSW'; call eq permOld,liveOld,'live/permanent old'; call eq permIc,liveIc,'live/permanent IC'
call eq permOld,'00040001E0000026','exact FF old'; call eq permPsw,'00040000000002CA','exact FF new'
say 'PASS test_program_operation_interrupt_ff_promotion_equivalence'
exit 0
eq: procedure
