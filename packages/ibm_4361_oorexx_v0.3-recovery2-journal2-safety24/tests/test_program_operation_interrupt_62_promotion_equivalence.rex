/* Safety18 exact X'62' microstate: live candidate -> rewind -> remove -> permanent. */
numeric digits 30
storage=.IBM370JournaledStorage~new(1048576,2048,'S18-62-PROMO-RAM'); m=.IBM4361Machine~new(1048576,storage); m~powerOn
...
s=.IBM4361JournalSession~new(m); s~beginInstruction; m~executor~installLiveMethod('architecturalOperationException',classifier); m~executor~installLiveMethod('programInterruptOperation',interrupt); call eq m~tick,'OK','live 62 executes'; livePsw=m~cpu~psw~rawHex; liveOld=m~storage~fetchHex(x2d('28'),8); liveIc=m~cpu~instructionCount; s~abortInstruction
...
m~executor~removeLiveMethod('architecturalOperationException'); m~executor~removeLiveMethod('programInterruptOperation'); call eq m~executor~hasMethod('OP62'),0,'no OP62'
s~beginInstruction; call eq m~tick,'OK','permanent 62 executes'; permPsw=m~cpu~psw~rawHex; permOld=m~storage~fetchHex(x2d('28'),8); permIc=m~cpu~instructionCount; s~commitInstruction
call eq permPsw,livePsw,'live/permanent PSW'; call eq permOld,liveOld,'live/permanent old'; call eq permIc,liveIc,'live/permanent IC'; call eq permOld,'00040001A0000054','exact old'; call eq permPsw,'00040000000002CA','exact new'
say 'PASS test_program_operation_interrupt_62_promotion_equivalence'; exit 0
