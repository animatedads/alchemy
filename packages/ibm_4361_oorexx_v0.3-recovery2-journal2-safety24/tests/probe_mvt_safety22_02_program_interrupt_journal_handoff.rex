/* Safety22: exact X'02' @ X'6E' operation-exception trial; no OP02. */
numeric digits 30
parse arg mediaPath log
if mediaPath='' then mediaPath='/mnt/data/s370_media/MVTRES.350'
if log='' then log='evidence/mvt_safety22_02_program_interrupt_journal_handoff.log'
...
do n=1 to 900000
  ...
  if st<>'OK' then do
    ...
    if ia=x2d('6E') & inst~left(2)='02' & m~cpu~instructionCount=534665 then found=1
    leave
  end
end
if \found then do; call emit 'FAIL expected 02 frontier not reached'; exit 2; end
...
js=.IBM370JournaledStorage~new(16777216,2048,'MVT-S22-02-RAM'); jm=.IBM4361Machine~new(16777216,js)
...
if first<>'UNSUPPORTED' then do; call emit 'FAIL first 02 not unsupported'; exit 5; end
classifier="numeric digits 30; use strict arg raw; op=raw~left(2)~translate; return op='00' | op='FF' | op='C4' | op='62' | op='65' | op='02'"
call emit 'PATCH classifier='||jm~executor~installLiveMethod('architecturalOperationException',classifier)||' OP02='||jm~executor~hasMethod('OP02')
...
old=jm~storage~fetchHex(x2d('28'),8); call emit 'RETRY ST='||retry||' IA='||d2x(jm~cpu~psw~instructionAddress,6)||' IC='||jm~cpu~instructionCount||' PSW='||jm~cpu~psw~rawHex||' PGMOLD='||old||' PGMNEW='||jm~storage~fetchHex(x2d('68'),8)||' OP02='||jm~executor~hasMethod('OP02')
if retry<>'OK' | old<>'0004000160000070' | jm~cpu~psw~rawHex<>'00040000000002CA' then do; call emit 'FAIL 02 operation retry'; exit 7; end
pst='OK'
do k=1 to 100000
...
call emit 'END'; call lineout log; say 'PASS probe_mvt_safety22_02_program_interrupt_journal_handoff log='log; exit 0
