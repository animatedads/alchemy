/* Safety21: exact X'65' @ X'66' operation-exception trial; no OP65. */
numeric digits 30
parse arg mediaPath log
if mediaPath='' then mediaPath='/mnt/data/s370_media/MVTRES.350'
if log='' then log='evidence/mvt_safety21_65_program_interrupt_journal_handoff.log'
...
do n=1 to 900000
  ...
  if st<>'OK' then do
    ...
    if ia=x2d('66') & inst~left(2)='65' & m~cpu~instructionCount=532961 then found=1
    leave
  end
end
if \found then do; call emit 'FAIL expected 65 frontier not reached'; exit 2; end
...
js=.IBM370JournaledStorage~new(16777216,2048,'MVT-S21-65-RAM'); jm=.IBM4361Machine~new(16777216,js)
...
if first<>'UNSUPPORTED' then do; call emit 'FAIL first 65 not unsupported'; exit 5; end
classifier="numeric digits 30; use strict arg raw; op=raw~left(2)~translate; return op='00' | op='FF' | op='C4' | op='62' | op='65'"
call emit 'PATCH classifier='||jm~executor~installLiveMethod('architecturalOperationException',classifier)||' OP65='||jm~executor~hasMethod('OP65')
...
old=jm~storage~fetchHex(x2d('28'),8); call emit 'RETRY ST='||retry||' IA='||d2x(jm~cpu~psw~instructionAddress,6)||' IC='||jm~cpu~instructionCount||' PSW='||jm~cpu~psw~rawHex||' PGMOLD='||old||' PGMNEW='||jm~storage~fetchHex(x2d('68'),8)||' OP65='||jm~executor~hasMethod('OP65')
if retry<>'OK' | old<>'00040001A000006A' | jm~cpu~psw~rawHex<>'00040000000002CA' then do; call emit 'FAIL 65 operation retry'; exit 7; end
pst='OK'
do k=1 to 100000
...
call emit 'END'; call lineout log; say 'PASS probe_mvt_safety21_65_program_interrupt_journal_handoff log='log; exit 0
