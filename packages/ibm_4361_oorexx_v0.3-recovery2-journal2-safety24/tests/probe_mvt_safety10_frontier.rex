...
if log='' then log='/mnt/data/mvt_safety10_permanent_frontier.log'
...
do n=1 to 300000
 ...
 if ic>=511890 then call emit 'PRE IC='||ic||' IA='||d2x(ia,6)||' INST='||inst||' PSW='||m~cpu~psw~rawHex||' R2='||d2x(m~cpu~gpr(2),8)||' R0='||d2x(m~cpu~gpr(0),8)||' CC='||m~cpu~psw~conditionCode
 st=m~tick
