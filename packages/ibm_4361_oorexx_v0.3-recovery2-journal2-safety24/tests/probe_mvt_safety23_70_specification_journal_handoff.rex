/* Safety23: exact X'70' @ X'82' invalid-FPR STE specification trial; no OP70. */
numeric digits 30
parse arg mediaPath log
if mediaPath='' then mediaPath='/mnt/data/s370_cont/media/MVTRES.350'
if log='' then log='evidence/mvt_safety23_70_specification_journal_handoff.log'
...
/* Shadow Safety23 permanent classification so the historical invocation is
 * first observed exactly as pre-promotion code would have observed it. */
...
do n=1 to 900000
  ...
  if st<>'OK' then do
    ...
    if ia=x2d('82') & inst~left(4)='70E0' & m~cpu~instructionCount=539172 then found=1
    leave
  end
end
if \found then do; call emit 'FAIL expected invalid STE frontier not reached'; exit 2; end
...
js=.IBM370JournaledStorage~new(16777216,2048,'MVT-S23-70-RAM'); jm=.IBM4361Machine~new(16777216,js)
...
call truth jm~executor~installLiveMethod('architecturalSpecificationException',oldClassifier),'install pre-Safety23 classifier on journal machine'
...
first=jm~tick; call emit 'FIRST ST='||first||' IA='||d2x(jm~cpu~psw~instructionAddress,6)||' IC='||jm~cpu~instructionCount||' LAST='||jm~executor~lastInstruction
if first<>'UNSUPPORTED' then do; call emit 'FAIL first 70 not unsupported'; exit 5; end
call emit 'PATCH classifier='||jm~executor~installLiveMethod('architecturalSpecificationException',newClassifier)||' OP70='||jm~executor~hasMethod('OP70')
...
old=jm~storage~fetchHex(x2d('28'),8); call emit 'RETRY ST='||retry||' IA='||d2x(jm~cpu~psw~instructionAddress,6)||' IC='||jm~cpu~instructionCount||' PSW='||jm~cpu~psw~rawHex||' PGMOLD='||old||' PGMNEW='||jm~storage~fetchHex(x2d('68'),8)||' OP70='||jm~executor~hasMethod('OP70')
if retry<>'OK' | old<>'0004000690000086' | jm~cpu~psw~rawHex<>'00040000000002CA' then do; call emit 'FAIL 70 specification retry'; exit 7; end
/* Withdraw the object-scope candidate; forward progress now uses permanent source. */
call truth jm~executor~removeLiveMethod('architecturalSpecificationException'),'remove live classifier after proof'
pst='OK'
do k=1 to 100000
...
call emit 'END'; call lineout log; say 'PASS probe_mvt_safety23_70_specification_journal_handoff log='log; exit 0
