/* Safety18: exact X'62' @ X'50' operation-exception trial; no OP62. */
numeric digits 30
parse arg mediaPath log
if mediaPath='' then mediaPath='/mnt/data/mvt_media18/MVTRES.350'
if log='' then log='/mnt/data/mvt_safety18_62_program_interrupt_journal_handoff.log'
...
do n=1 to 800000
  ...
  if st<>'OK' then do
    ...
    if ia=x2d('50') & inst~left(2)='62' & m~cpu~instructionCount=529092 then found=1
    leave
  end
end
if \found then do; call emit 'FAIL expected 62 frontier not reached'; exit 2; end
...
js=.IBM370JournaledStorage~new(16777216,2048,'MVT-S18-62-RAM'); jm=.IBM4361Machine~new(16777216,js)
...
if first<>'UNSUPPORTED' then do; call emit 'FAIL first 62 not unsupported'; exit 5; end
classifier="numeric digits 30; use strict arg raw; op=raw~left(2)~translate; return op='00' | op='FF' | op='C4' | op='62'"
call emit 'PATCH classifier='||jm~executor~installLiveMethod('architecturalOperationException',classifier)||' OP62='||jm~executor~hasMethod('OP62')
...
old=jm~storage~fetchHex(x2d('28'),8); call emit 'RETRY ST='||retry||' IA='||d2x(jm~cpu~psw~instructionAddress,6)||' IC='||jm~cpu~instructionCount||' PSW='||jm~cpu~psw~rawHex||' PGMOLD='||old||' PGMNEW='||jm~storage~fetchHex(x2d('68'),8)||' OP62='||jm~executor~hasMethod('OP62')
if retry<>'OK' | old<>'00040001A0000054' | jm~cpu~psw~rawHex<>'00040000000002CA' then do; call emit 'FAIL 62 operation retry'; exit 7; end
pst='OK'
do k=1 to 100000
...
call emit 'END'; call lineout log; say 'PASS probe_mvt_62_program_interrupt_journal_handoff log='log; exit 0
