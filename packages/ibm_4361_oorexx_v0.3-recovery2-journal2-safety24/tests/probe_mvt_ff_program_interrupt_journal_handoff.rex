/* Safety16: reach exact FF @ X'20' under sealed safety15 semantics, hand off
 * to journal RAM, then trial FF as an architectural operation exception.
 * No OPFF is installed. */
numeric digits 30
parse arg mediaPath log
if mediaPath='' then mediaPath='/mnt/data/mvt_media16/mvtres.350'
if log='' then log='/mnt/data/mvt_safety16_ff_program_interrupt_journal_handoff.log'
...
do n=1 to 700000
  ...
  if st<>'OK' then do
    ...
    if ia=x2d('20') & inst~left(2)='FF' & m~cpu~instructionCount=521152 then found=1
    leave
  end
end
if \found then do; call emit 'FAIL expected FF frontier not reached'; exit 2; end
call emit 'LOWCORE SVCOLD='||m~storage~fetchHex(x2d('20'),8)||' PGMOLD='||m~storage~fetchHex(x2d('28'),8)||' SVCNEW='||m~storage~fetchHex(x2d('60'),8)||' PGMNEW='||m~storage~fetchHex(x2d('68'),8)
...
js=.IBM370JournaledStorage~new(16777216,2048,'MVT-S16-FF-RAM'); jm=.IBM4361Machine~new(16777216,js)
...
if first<>'UNSUPPORTED' then do; call emit 'FAIL first FF not unsupported'; exit 5; end
classifier="numeric digits 30; use strict arg raw; op=raw~left(2)~translate; return op='00' | op='FF'"
call emit 'PATCH classifier='||jm~executor~installLiveMethod('architecturalOperationException',classifier)||' OPFF='||jm~executor~hasMethod('OPFF')
jm~cpu~setGpr(5,x2d('DEADBEEF')); jm~storage~storeHex(x2d('1234'),'A55A'); session~abortInstruction
call emit 'REWIND IA='||d2x(jm~cpu~psw~instructionAddress,6)||' IC='||jm~cpu~instructionCount||' PSW='||jm~cpu~psw~rawHex||' OLD='||jm~storage~fetchHex(x2d('28'),8)||' R5='||d2x(jm~cpu~gpr(5),8)||' M1234='||jm~storage~fetchHex(x2d('1234'),2)||' CLASS='||jm~executor~hasMethod('architecturalOperationException')
...
old=jm~storage~fetchHex(x2d('28'),8)
call emit 'RETRY ST='||retry||' IA='||d2x(jm~cpu~psw~instructionAddress,6)||' IC='||jm~cpu~instructionCount||' PSW='||jm~cpu~psw~rawHex||' PGMOLD='||old||' PGMNEW='||jm~storage~fetchHex(x2d('68'),8)||' OPFF='||jm~executor~hasMethod('OPFF')
if retry<>'OK' | old<>'00040001E0000026' | jm~cpu~psw~rawHex<>'00040000000002CA' then do; call emit 'FAIL FF operation retry'; exit 7; end
pst='OK'
do k=1 to 50000
...
call emit 'END'; call lineout log; say 'PASS probe_mvt_ff_program_interrupt_journal_handoff log='log; exit 0
