...
## Historical external observations

If a real ChatGPT/Codex incident occurred before native projection receipts were available, record it with `COGNITIVE.DOGFOOD.OBSERVE`. Keep the diagnosis precise: for example, the Safety29 transcript contained a FIFO request/reply correlation failure and a normal-ask cognitive-context bypass; it was not itself evidence of a Cognitive Continuity projection failure. These observations are measurement-only and flow to Qwen as `dogfoodObservations`.
