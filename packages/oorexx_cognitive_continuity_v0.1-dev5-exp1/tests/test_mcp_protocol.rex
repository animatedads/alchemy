...
ignore=p~grant('codex:mcp','cognitive.projection.outcome','project:mcp')
...
r=protocol~handle(headers,body,'codex:mcp'); call eq 200,r~status,'list status'; parsed=.json~fromJSON(r~jsonText); call eq 12,parsed['result']['tools']~items,'tool count'
