...
ignore=p~grant('cognitive:scheduler','cognitive.context.project','project:learn')
ignore=p~grant('cognitive:scheduler','cognitive.context.explain','project:learn')
...
bundle=svc~dispatch('COGNITIVE.LEARNING.BUNDLE',args,'cognitive:scheduler')
call must bundle~ok,'bundle'
req=.CognitiveLearningRequestBuilder~fromBundle(bundle~value,'HOURLY','NORMAL')
call eq 'cognitive.learning.request/2',req['schema'],'request schema'
call eq 2,req['recordCount'],'request count'
call eq 0,req['projectionReceipts']~items,'request receipts initially empty'
call eq 0,req['projectionOutcomes']~items,'request outcomes initially empty'
