...
for spec in \
  ...
  "ALCHEMY_OBJECTS_ROOT:$ALCHEMY_OBJECTS_ROOT/AlchemyObject.cls" \
...
parts=("$ROOT/src" "$AI_ACCESS_ROOT/src" "$RUNTIME_REGISTRY_ROOT/src" "$ALCHEMY_OBJECTS_ROOT" "$CRYPTO_SRC")
if [[ -n "$WLU_ROOT" ]]; then
...
export RUNTIME_REGISTRY_ROOT

echo "=== test_ai_tool_broker.rex ==="
"$REXX" "$ROOT/tests/test_ai_tool_broker.rex" "$ROOT"
if [[ -n "$WLU_ROOT" ]]; then
  echo "=== test_ai_tool_broker_wlu.rex ==="
  "$REXX" "$ROOT/tests/test_ai_tool_broker_wlu.rex" "$ROOT"
else
  echo "=== test_ai_tool_broker_wlu.rex: SKIP (WLU_ROOT not set) ==="
fi
