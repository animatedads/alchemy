...
static size_t REXXENTRY py_object_call(CONSTANT_STRING, size_t argc, PCONSTRXSTRING argv, CONSTANT_STRING, PRXSTRING result)
{
    ...
    if (argc >= 2) {
        ...
        std::string mn(argv[1].strptr, argv[1].strlength);
        uint64_t h = strtoull(hs.c_str(), nullptr, 10);
        PyObject *obj = nullptr;
        { std::lock_guard<std::mutex> lock(py_object_mutex); auto i=py_objects.find(h); if(i!=py_objects.end()) obj=i->second; }
        if (obj != nullptr) {
...
{
    PyObject *obj; const char *factory; if(!PyArg_ParseTuple(args,"Os",&obj,&factory)) return nullptr;
    if(!ensure_interpreter()) return PyErr_Format(PyExc_RuntimeError,"RexxCreateInterpreter failed");
    ...
    uint64_t ph; { std::lock_guard<std::mutex> lock(py_object_mutex); ph=next_py_handle++; Py_INCREF(obj); py_objects[ph]=obj; }
    ...
    RexxObjectPtr o=tc->CallProgram(factory,a);
    if(o==NULLOBJECT || tc->CheckCondition()){tc->ClearCondition(); std::lock_guard<std::mutex> lock(py_object_mutex); auto i=py_objects.find(ph); if(i!=py_objects.end()){Py_DECREF(i->second);py_objects.erase(i);} return PyErr_Format(PyExc_RuntimeError,"ooRexx Python proxy factory failed");}
...
static PyObject *release_python_object(PyObject *, PyObject *args)
{
    ...
    std::lock_guard<std::mutex> lock(py_object_mutex); auto i=py_objects.find(h); if(i!=py_objects.end()){Py_DECREF(i->second);py_objects.erase(i);} Py_RETURN_NONE;
}
