...
if [ ! -d "$REPO/.git" ]; then
  ...
  git -C "$REPO" fetch origin "$FETCH_REFSPEC"
...
[Service]
Type=simple
ExecStart=/usr/bin/python3 $REPO/packages/alchemy_autobuild_v0.4.2/alchemy_autobuild.py --config $ROOT/config.json
Restart=on-failure
RestartSec=2
...
printf '%s\n' "Installed Alchemy Autobuild v0.4.2 service definition"
