#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")"
NOSQL_CLS="${1:-}"
if [[ -z "$NOSQL_CLS" ]]; then
  echo "usage: $0 /path/to/NoSQLServer.cls" >&2
  exit 2
fi
NOSQL_CLS="$(readlink -f "$NOSQL_CLS")"
TMP="$(mktemp ./test_librarian_table_feed_v068.XXXXXX.rex)"
ROOT="$(mktemp -d /tmp/librarian_table_feed_v09.XXXXXX)"
trap 'rm -f "$TMP"; rm -rf "$ROOT"' EXIT
python3 - "$NOSQL_CLS" "$TMP" <<'PY'
from pathlib import Path
import sys
src=Path('test_librarian_table_feed_pipeline_v068.rex.in').read_text()
Path(sys.argv[2]).write_text(src.replace('__NOSQLSERVER_CLS__', sys.argv[1]))
PY
rexx "$TMP" "$ROOT"
