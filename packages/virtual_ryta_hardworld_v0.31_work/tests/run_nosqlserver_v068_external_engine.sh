...
TMP_REX="$(mktemp "$HERE/.nosql_v068_external.XXXXXX.rex")"
ROOT="$(mktemp -d /tmp/virtual_ryta_nosql_v07.XXXXXX)"
cleanup(){ rm -f "$TMP_REX"; rm -rf "$ROOT"; }
trap cleanup EXIT
python3 - "$HERE/test_nosqlserver_v068_external_engine.rex.in" "$TMP_REX" "$NOSQL_CLS" <<'PY'
