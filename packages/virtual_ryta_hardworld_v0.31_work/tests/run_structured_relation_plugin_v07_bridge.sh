#!/usr/bin/env bash
set -euo pipefail
PLUGIN_ROOT=${1:?usage: $0 /path/to/structured_relation_plugin_v0.7}
SELF_DIR=$(cd "$(dirname "$0")" && pwd)
RYTA_ROOT=$(cd "$SELF_DIR/.." && pwd)
TMP=$(mktemp "$SELF_DIR/structured-ryta-bridge.XXXXXX.rex")
trap 'rm -f "$TMP"' EXIT
python3 - "$SELF_DIR/test_structured_relation_plugin_v07_bridge.rex.in" "$TMP" "$PLUGIN_ROOT" "$RYTA_ROOT" <<'PY'
