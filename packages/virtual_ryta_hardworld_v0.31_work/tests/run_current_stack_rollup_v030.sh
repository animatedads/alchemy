...
printf '%s\n' '=== v0.30 Alchemy v0.8 preferred adoption ==='
cd "$HERE"; "$REXX" test_alchemy_v08_adoption_v030.rex
printf '%s\n' '=== v0.30 Alchemy v0.8 / Logging v0.5 cooperative interposition ==='
cd "$HERE"; "$REXX" test_alchemy_v08_logging_interposition_v029.rex
printf '%s\n' '=== v0.30 Database Core v0.43 transaction/retry evidence ==='
cd "$HERE"; REXX_PATH="$DATABASE_CORE_ROOT:$REXX_PATH" "$REXX" test_database_transaction_evidence_v030.rex
printf '%s\n' '=== v0.30 Legal reducer guard ==='
...
printf '%s\n' '=== v0.30 native live Legal authority compatibility ==='
...
printf '%s\n' '=== v0.30 authenticated ledger adversaries ==='
QUEUE_FABRIC_ROOT="$QUEUE_FABRIC_ROOT" "$HERE/run_queue_authority_ledger_auth_v027.sh"
printf '%s\n' '=== v0.30 authenticated Legal completed-work recovery ==='
...
printf '%s\n' 'CURRENT STACK ROLL-UP V0.30-WORK: OK'
