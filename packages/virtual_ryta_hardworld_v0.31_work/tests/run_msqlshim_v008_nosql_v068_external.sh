...
TMP="$(mktemp -d /tmp/virtual_ryta_wire_v07.XXXXXX)"
DBROOT="$TMP/db"
cleanup(){
...
# Arrange the paths exactly as ooRexx ::REQUIRES expects when cwd is TMP/tests.
...
cp "$ROOT/tests/mysql_wire_algrel_v068_external_server.rex" "$TMP/tests/"
cat > "$DBROOT/database.yaml" <<'YAML'
name: msqlshim_algrel_v068_external
formatVersion: 1
tables: []
YAML
cd "$TMP/tests"
(rexx mysql_wire_algrel_v068_external_server.rex "$PORT" "$DBROOT" > "$TMP/server.log" 2>&1 & echo $! > "$TMP/server.pid")
...
cat "$TMP/server.log"
python3 "$ROOT/tests/mysql_wire_algrel_v068_external_client.py" "$PORT"
