say 'LEGAL V0.10 AUTHORITY LIFETIME V0.26 START'
call main
exit 0
...
main:
  ...
  artifactId = 'hardworld-v026:legal-effect-v010:lifetime'
  call MustOk verifier~pin(artifactId, bundle~sourceLines), 'pin legal runtime closure'
  artifact = .RuntimeArtifact~new('legal.effect.v010.retained', 'LEGAL_RULES', '0.10', artifactId, 'DemoLegalRules', bundle~sourceLines, .LegalEffectBuild~API_VERSION, 'hardworld-v026-lifetime-proof')
  ...
  call AssertEqual 'COMPLETE', ledger~stateFor(ackFirst~workIdentity~executionKey), 'v2 ledger durable COMPLETE recorded'
  ...
  ackRecovered = .QueueAuthorityPromotionExecutor~execute(ackManager, 'Q.LEGAL.ACKRECOVERY', ackPackage2, ackPackage2~claimToken, 'admin', publisherSet, ackWorld, 'NODE-B/LEGAL-ACKRECOVERY', ledger, .nil, revokedRevalidator)
  ...
  say 'LEGAL V0.10 AUTHORITY LIFETIME V0.26: OK'
  return
