...
if [[ -z "$MSQL_ROOT" || ! -f "$MSQL_ROOT/src/MySQLWireServer.cls" ]]; then
  echo "usage: $0 /path/to/msqlshim_v0.10 /path/to/nosqlserver_v0.70/src/NoSQLServer.cls [port]" >&2
  exit 2
fi
...
TMP="$(mktemp -d /tmp/virtual_ryta_cursor_v013.XXXXXX)"
DBROOT="$TMP/db"
cleanup(){
...
cp "$ROOT/tests/mysql_wire_algrel_v070_cursor_server.rex" "$TMP/tests/"
cat > "$DBROOT/database.yaml" <<'YAML'
name: msqlshim_algrel_v070_cursor
formatVersion: 1
tables: []
YAML
cd "$TMP/tests"
(rexx mysql_wire_algrel_v070_cursor_server.rex "$PORT" "$DBROOT" > "$TMP/server.log" 2>&1 & echo $! > "$TMP/server.pid")
pid="$(cat "$TMP/server.pid")"
for _ in $(seq 1 100); do
  grep -q 'MSQL V010 V070 ALGREL CURSOR READY' "$TMP/server.log" && break
  kill -0 "$pid" 2>/dev/null || { cat "$TMP/server.log"; exit 1; }
  sleep 0.1
done
cat "$TMP/server.log"
python3 "$ROOT/tests/mysql_wire_algrel_v070_cursor_client.py" "$PORT"
