/* Test server: stock NoSQLServer v0.69 + lazy Algorithm Relation external engine + msqlshim v0.10 prepared cursors. */
parse arg port root
...
server~engine = fed
say 'MSQL V010 ALGREL CURSOR READY port=' || port || ' invocations=' || provider~invocationCount
server~serve
