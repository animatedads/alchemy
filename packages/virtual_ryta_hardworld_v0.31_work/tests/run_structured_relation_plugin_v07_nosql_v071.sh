#!/usr/bin/env bash
set -euo pipefail
PLUGIN_ROOT=${1:?usage: $0 /path/to/structured_relation_plugin_v0.7 /path/to/NoSQLServer.cls}
NOSQL_CLS=${2:?usage: $0 /path/to/structured_relation_plugin_v0.7 /path/to/NoSQLServer.cls}
SELF_DIR=$(cd "$(dirname "$0")" && pwd)
RYTA_ROOT=$(cd "$SELF_DIR/.." && pwd)
PLUGIN_ROOT=$(realpath "$PLUGIN_ROOT")
NOSQL_CLS=$(realpath "$NOSQL_CLS")
TMP=$(mktemp "$SELF_DIR/.structured-v07-nosql.XXXXXX.rex")
DBROOT=$(mktemp -d /tmp/structured-v07-nosql.XXXXXX)
cleanup(){ rm -f "$TMP"; rm -rf "$DBROOT"; }
trap cleanup EXIT
python3 - "$SELF_DIR/test_structured_relation_plugin_v07_nosql_v071.rex.in" "$TMP" "$PLUGIN_ROOT" "$NOSQL_CLS" "$RYTA_ROOT" <<'PY'
