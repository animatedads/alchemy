...
TMP_REX="$(mktemp "$HERE/.nosql_v070_dbcore.XXXXXX.rex")"
ROOT="$(mktemp -d /tmp/virtual_ryta_nosql_v070_dbcore.XXXXXX)"
cleanup(){ rm -f "$TMP_REX"; rm -rf "$ROOT"; }
trap cleanup EXIT
python3 - "$HERE/test_nosqlserver_v070_dbcore_algrel.rex.in" "$TMP_REX" "$NOSQL_CLS" <<'PY'
