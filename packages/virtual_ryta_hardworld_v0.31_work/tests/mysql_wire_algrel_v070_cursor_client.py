#!/usr/bin/env python3
"""msqlshim v0.10 prepared cursor over v0.70 provider-neutral Algorithm Relation metadata."""
...
def stats(s):
    rows=text_query(s,'SELECT invocations,metadata_calls FROM algrel_provider_stats')
...
assert stats(s)==(0,0)
print('PASS two COM_STMT_PREPARE relations provider invocations=0 metadata_factory_calls=0')

ccols=execute_cursor(s,stmt); assert ccols==2
assert stats(s)==(1,0)
print('PASS COM_STMT_EXECUTE materialized once; legacy metadata factory still unused')
...
assert stats(s)==(1,0)
print('PASS second prepared relation reuses materialization; metadata factory calls remain 0')
...
assert stats(s)==(1,0)
print('PASS cursor reset/re-execute reuses Algorithm Relation; metadata factory calls remain 0')

sendpkt(s,0,b'\x19'+struct.pack('<I',stmt))
sendpkt(s,0,b'\x19'+struct.pack('<I',trace_stmt))
sendpkt(s,0,b'\x01'); s.close()
print('MYSQL WIRE V0.10 PREPARED CURSOR ALGORITHM RELATION V0.70: OK')
