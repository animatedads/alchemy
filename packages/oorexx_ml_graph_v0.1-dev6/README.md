...
## dev6: raster/image overlays and Vision/V5V surfaces

Dev6 adds a sibling semantic image scene for technical overlays.  The base raster remains upstream evidence; drawing never mutates it.  This is deliberately separate from `MLGraph` axis/series semantics because an image plus annotations is not a numerical plot.

```text
VisionSurface / RGBA source evidence
        |
        v
MLImageScene
  immutable base source
  ordered MLImageLayer objects
    LINE / POLYLINE / POLYGON
    RECTANGLE / ELLIPSE / CIRCLE
    MARKER / TEXT
  stroke/fill alpha + style opacity
  PIXEL or NORMALIZED coordinates
        |
        +--> deterministic SVG reference renderer
        `--> Foreign Runtime -> Python -> Pillow renderer (optional)
```

`MLVisionSurfaceImageSource` binds directly to the public ooRexx Vision `VisionSurface` contract.  Width and height are taken from the surface, so there is no codec-specific fixed frame size.  If the surface has a `VisionColourModel`, its indexed value is reconstructed through that model; otherwise a deterministic greyscale presentation is used.  The original `VisionSurface` remains the source/evidence object.

V5V temporal decoding remains owned by Vision.  Graph accepts the decoded `VisionSurface`; it does not parse V5V wire records or reinterpret palette/temporal semantics.  High-resolution `VisionRegionMaterial` remains a separate evidence-escalation path.

Rendering scale is presentation-only.  For example, a 37x23 Vision surface rendered with scale 10 produces a 370x230 output while all overlay coordinates remain in the original 37x23 coordinate system.
...
Image/raster objects added in dev6:

- `MLImageColour` — RGB plus alpha.
- `MLImageStyle` — independent stroke/fill colour+alpha, stroke width, overall opacity, line cap/join and fill rule.
- `MLImageVertex` and `MLImageGeometry`.
- `MLImageLine`, `MLImagePolyline`, `MLImagePolygon`.
- `MLImageRectangle`, `MLImageEllipse`, `MLImageCircle`.
- `MLImageMarker` (`CIRCLE`, `SQUARE`, `CROSS`) and `MLImageText`.
- `MLImageLayer` — ordered primitives, layer opacity/visibility and retained evidence.
- `MLImageSource`, `MLImageArraySource`, `MLVisionSurfaceImageSource`.
- `MLImageScene` — immutable source plus overlay layers and metadata.
- `MLImageRenderOptions` — presentation scale/base/clip policy.
- `MLImageRenderer`, `MLImageSvgRenderer`, `MLImagePillowRenderer` (optional), `MLImageRenderers`.
...
Vision/V5V image adapter qualification:

- ooRexx Vision v0.1-dev14-v5v-codec-highres (`VisionSurface` public contract)

Optional Python renderers:

- ooRexx Foreign Runtime v0.22.6
- matplotlib provider: Python with matplotlib (previous qualification: Python 3.13.5, matplotlib 3.10.8, NumPy 2.3.5)
- Pillow provider: Python with Pillow (`PIL`)

No Python/native handle is stored in `MLGraph`, `MLImageScene`, overlay primitives, graph evidence, Vision evidence, or ML learned state.
