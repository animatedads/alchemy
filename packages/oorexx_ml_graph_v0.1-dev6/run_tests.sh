...
export REXX_PATH="$ROOT/src${ML_ROOT:+:$ML_ROOT/src:$ML_ROOT/lib}${VISION_ROOT:+:$VISION_ROOT/src}${FOREIGN_ROOT:+:$FOREIGN_ROOT/rexx}${REXX_PATH:+:$REXX_PATH}"
...
"$REXX_BIN" "$ROOT/tests/test_image_model.rex"
"$REXX_BIN" "$ROOT/tests/test_image_svg.rex" "$ROOT/qualification/test_mlimage.svg"
if [[ -n "$VISION_ROOT" ]]; then
  "$REXX_BIN" "$ROOT/tests/test_v5v_image_overlay.rex" "$ROOT/qualification/test_v5v_overlay.svg"
fi
...
if [[ -n "$FOREIGN_ROOT" ]]; then
  "$REXX_BIN" "$ROOT/tests/test_pillow_image.rex" "$ROOT/qualification/test_mlimage_pillow.png"
