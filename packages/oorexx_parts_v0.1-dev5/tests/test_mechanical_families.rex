...
do s over sizes
  ...
  if \b~parameters~hasIndex('HEAD_SHEAR_CAPACITY_N') then call fail 'bolt capacity'
  if b~parameter('HEAD_SHEAR_CAPACITY_N')<=0 then call fail 'bolt capacity positive'
end
