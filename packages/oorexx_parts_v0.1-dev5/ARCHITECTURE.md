...
Unknown parameters raise an error. Unsupported Rexx-tronics families raise an error. A catalogue entry must never silently choose a behavioural model that the consumer does not support. Unresolved material IDs raise at construction time.

## Parameterised families

Prefer factories (metricBolt, bearingDeepGroove, rectangularPlate, gt2Pulley, spurGear, …) over one-off objects. Geometry that is standard-defined is marked STANDARD_GEOMETRY or ENGINEERING_REFERENCE; approximate ratings carry explicit provenance notes.
