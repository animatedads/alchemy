...
## dev4 source consistency authority
Regular-file dispatch is fail-closed when Linux /proc evidence shows any foreign open descriptor or mmap. A quiet observation window plus unchanged nanosecond source identity yields OBSERVED_QUIESCENT only; it is not represented as application consistency. APPLICATION_CONSISTENT and SNAPSHOT_CONSISTENT are reserved for future explicit providers. Content verification and source consistency are independent evidence axes. Final sealing requires verified regular-file replicas to carry non-UNKNOWN consistency evidence.

## Multi-object consistency authority (dev6)

Application datasets are consistency groups, not bags of individually quiet
files. The authority sequence is: validate complete group -> acquire one
provider lease -> fence/capture all members -> independently verify each replica
-> release provider -> retain group/lease evidence. Failure to acquire a lease
is `WAITING_FOR_QUIESCENCE`, never implicit permission to perform a best-effort
copy. `APPLICATION_CONSISTENT` and `SNAPSHOT_CONSISTENT` remain distinct claims.
