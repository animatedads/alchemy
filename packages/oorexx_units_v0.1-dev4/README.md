...
### Engineering display units (dev3)
Common electronics/physics scales are first-class catalogue units. For example:
`uW`, `mW`, `W`, `kW`, `MW`; `uV`, `mV`, `V`, `kV`; `uA`, `mA`, `A`, `kA`; `mohm`, `ohm`, `kohm`, `Mohm`; `mF`, `uF`, `nF`, `pF`; `uH`, `mH`, `H`; and `Hz`, `kHz`, `MHz`, `GHz`.
Unicode micro spellings such as `µA`, `µF`, `µH`, `µV`, and `µW` are accepted on input.


## dev4 practical/scientific catalogue

Adds common non-SI and customary units used in engineering and everyday measurement: `psi`, `atm`, `Torr`, `mmHg`, `cc`, cubic inch/foot, acre, imperial and US liquid gallon/quart/pint/fluid-ounce families, nautical mile, knot, ounce, stone, tonne, short/long ton, calorie/kcal, BTU, pound-force, mechanical horsepower (`hp`) and metric horsepower (`PS`). `kph` is accepted as an alias for `km/h`. Bare `gallon`/`gal` is deliberately rejected because UK and US gallons are different units.
