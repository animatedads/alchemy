...
`AlchemyObject` provides inherited identity, lifecycle/usage telemetry, structured metadata, environmental and external requirements, method/result contracts, method data descriptions, declared instrumentation points, method performance telemetry, compliance score surfaces, disclosure-labelled state registration, fixed object-specific emitters, and cryptographically sealed introspection/snapshot messages.
...
## Method contracts, data descriptions and telemetry

`registerMethodContract()` describes purpose, argument surface and result contract. `describeMethodData()` adds structured INPUT/OUTPUT/RESULT descriptions including type, required status, description and disclosure classification.

`instrumentMethod()` can install an object-specific wrapper around an eligible method. It retains the original method as a private object alias and records calls, successes, failures, elapsed totals/min/max/last and result-contract failures. The wrapper preserves the original guarded/unguarded mode.
...
`instrumentRegisteredMethods()` attempts instrumentation for registered contracts and returns a result for every method, including explicit skip results for ineligible surfaces.
...
`declareHouseCompliance(claimedScore)` registers the `ALCHEMY-HOUSE-OBJECT-0.4` structural assessment. `complianceReport()` independently calculates the score, records claimed/calculated/max values and their delta, and returns the report only as capability-gated sealed evidence.

The current 100-point structural check covers metadata, surface contracts, requirements, instrumentation, method/state descriptions, configured evidence/security facilities and lifecycle evidence. It is a machine-checkable house standard, not a claim that the application semantics are correct.
...
A subclass with its own `INIT` should call `self~initAlchemy(...)`, then register its state slots, contracts, data descriptions, requirements, instrumentation points and compliance checks.
...
Project-host field validation on ooRexx 5.2.0 r0 completed all 22 tests in one
uninterrupted run (real 15.940s) and independently confirms that the static
`::REQUIRES` Security Manager supervision gap is present on that runtime too.
