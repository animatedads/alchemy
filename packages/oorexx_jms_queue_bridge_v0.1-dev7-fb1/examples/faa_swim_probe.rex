/* Bounded FAA/SCDS JMS readiness probe. No consumer is created. */
...
config = .JMSBridgeConfig~new("faa-swim-probe", "SOLACE", providerUrl, connectionFactory, remoteQueue, "", vpn, "", "", "", "", "queue-admin", .false, 1000, "REJECT", "com.solacesystems.jndi.SolJNDIInitialContextFactory", sessionMode, transportMode)
provider = .JMSBSFProvider~new(config, credentials)

result = provider~probe
if \result~ok then do
  say "JMS LIVE PROBE FAIL" result~code result~detail
  exit 1
end

say "JMS LIVE PROBE PASS" result~code result~detail
exit 0

missing: procedure
  use arg name
  say "JMS LIVE PROBE CONFIG MISSING" name
  exit 2
