...
parse arg jobPath sourcePath companionPath ref1Path ref2Path outDir bridgePath
if words(arg(1))<7 then do
  say 'usage: AudioSearchExperiment.rex JOB SOURCE COMPANION REF1 REF2 OUT BRIDGE'; exit 2
end
...
ctx=.AudioSearchNativeContext~new(bridgePath,sourcePath,companionPath,ref1Path,ref2Path)
...
runGA: procedure
  ...
  objective=.MLObjective~new('recovered-reference-distance','MINIMIZE','0.35*global + 0.40*window_median + 0.10*window_p25 + 0.15*temporal + 18*pre_limiter_over + 40*post_limiter_clip + low-silence penalty','recovered-reference resemblance; lower score is better; artefact weights require perceptual calibration')
...
runCheckpointedExploration: procedure
  ...
  objective=.MLObjective~new('recovered-reference-distance','MINIMIZE','0.35*global + 0.40*window_median + 0.10*window_p25 + 0.15*temporal + 18*pre_limiter_over + 40*post_limiter_clip + low-silence penalty','lower score is better; artefact weights require perceptual calibration')
...
writeResults: procedure
  use strict arg path,job,source,companion,align,shared,runMeta,candidateCount,results
  ...
  s~lineout('  "objective": "recovered-reference resemblance with global/window/temporal distance and artefact penalties",')
...
writeReviewCatalog: procedure
  ...
  do i=1 to results~items
    ...
    s~lineout(candidateId||tab||laneId||tab||i||tab||canonicalNumber(r['score'])||tab||r['output']||tab||r['pcm_hash64']~string||tab||'recovered-reference-distance'||tab||chain)
  end
  s~close
  return
