...
- `inflate(bytes, maxOutputBytes=.nil)` -> standalone Stored/Fixed/Dynamic DEFLATE decoding without preset history. When `maxOutputBytes` is supplied, decoding fails before output can exceed that hard ceiling.
...
There is intentionally no `compress.deflate.dynamic` capability in dev4.
