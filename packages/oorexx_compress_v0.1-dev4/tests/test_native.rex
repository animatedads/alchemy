...
/* Bounded decode must fail before an archive bomb can outgrow its declared size. */
boundedPlain = copies('bounded-output-', 1000)
boundedRaw = codec~deflateFixed(boundedPlain)
if codec~inflate(boundedRaw, length(boundedPlain)) \== boundedPlain then call fail 'bounded DEFLATE exact-limit decode failed'
if \expectLimitedReject(boundedRaw, length(boundedPlain) - 1) then call fail 'bounded DEFLATE limit was not enforced'
...
expectLimitedReject: procedure
  signal on syntax name rejected
  use strict arg bytes, maxBytes
  codec = .Compress~new
  discard = codec~inflate(bytes, maxBytes)
  return .false
rejected:
  return .true
