...
## v0.16.2 exported Python retain handle

The v0.15 reverse-argument demo and v0.16 UNKNOWN-collision demo both need to
retain an arbitrary Python receiver before handing its opaque handle to Rexx.
The native module previously exported `release_python_object` but not the
symmetric retain operation.

v0.16.2 exports `retain_python_object(object)`.  It INCREFs the object and
returns the existing opaque pointer-handle representation used by this POC.
`release_python_object(handle)` remains the matching lifetime operation.

## v0.16.3 UNKNOWN dispatch visibility repair

The relocated original UNKNOWN worked, proving the Method-object relocation and
alias installation had succeeded.  The first projected message then failed
because the dynamically compiled UNKNOWN source tried to invoke `PYCALL`
directly from a sibling Rexx package.

v0.16.3 routes generated UNKNOWN dispatch through the receiver's
`bridgePythonCall` method.  That method lives in the same class/package as the
existing UNKNOWN experiment and owns the call to the registered PYCALL routine.
The original UNKNOWN Method-object relocation/fallback design is unchanged.

## v0.16.4 projected UNKNOWN dispatch moved to native seam

v0.16.3 proved that simply hiding PYCALL behind another Rexx method did not
solve the projected-message failure.  v0.16.4 removes that package/routine
visibility dependency from the experiment.

The dynamically installed UNKNOWN now has only two responsibilities:
- for a Python-profile message, return a private `!PYDISPATCH:<message>` request;
- otherwise invoke the relocated original UNKNOWN Method object.

The native `send0_text` test seam recognizes the private request, reads the
object's retained `FOREIGNHANDLE`, and calls the Python method directly.

This deliberately keeps the question under test narrow: can an existing
UNKNOWN be preserved, relocated, overlaid, and forwarded correctly?  General
Rexx-to-Python callback routing remains a separate bridge concern.
