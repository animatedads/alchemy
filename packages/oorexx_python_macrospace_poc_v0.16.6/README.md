...
## v0.16.5 regression repair

v0.16.4 used an unsafe textual replacement and patched the first matching
native string-return tail rather than the `send0_text()` function specifically.
That contaminated the established animal factory path, which is why
`object_demo.py` failed before reaching the UNKNOWN experiment.

v0.16.5 is rebuilt from v0.16.3 and replaces `send0_text()` strictly by its
function boundaries.  Static regression guards verify that `create_animal`,
`create_guarded`, and `create_stem` contain no `!PYDISPATCH` experiment code.

## v0.16.6
Repairs the factory package layout: the collision class now shares factory.rex; the appended sibling ::requires and temporary PYDISPATCH detour are removed.
