...
with ctx.wrap_socket(conn,server_side=True) as s:
    ...
    f.write(b'* OK [CAPABILITY IMAP4rev1 UIDPLUS MOVE] move-test ready\r\n')
    while True:
        line=f.readline()
        if not line: break
        text=line.decode('utf-8','replace').rstrip('\r\n'); parts=text.split(' ',1); tag=parts[0]; cmd=parts[1] if len(parts)>1 else ''
        u=cmd.upper()
        if u=='CAPABILITY': f.write(b'* CAPABILITY IMAP4rev1 UIDPLUS MOVE\r\n'+tag.encode()+b' OK capability\r\n')
        ...
        elif u.startswith('UID MOVE 7 '): f.write(b'* 1 EXPUNGE\r\n'+tag.encode()+b' OK [COPYUID 20 7 70] moved\r\n')
