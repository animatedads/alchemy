call main
exit 0
main:
  seed = "9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60"
  publicKey = "d75a980182b10ab7d54bfed3c964073a0ee172f3daa62325af021a68f707511a"
  signature = "e5564300c360ac729086e2cc806e828a84877f1eb8e5d974d873e065224901555" ||,
              "fb8821590a33bacc61e39701cf9b46bd25bf5f0595bbe24655141438e7a100b"
  kp = .Ed25519~keypair(seed)
  call assertEq publicKey, kp["public"], "RFC 8032 public key"
  call assertEq signature, .Ed25519~sign("", seed), "RFC 8032 signature"
  call assertTrue .Ed25519~verify("", signature, publicKey), "RFC 8032 verify"
  call assertFalse .Ed25519~verify("x", signature, publicKey), "message tamper"
  bad = "f" || signature~substr(2)
  call assertFalse .Ed25519~verify("", bad, publicKey), "signature tamper"
  say "PASS test_ed25519"
  return
assertEq: procedure; use arg e,a,label; if e \== a then raise syntax 88.900 array(label || " expected=" || e || " actual=" || a); return
assertTrue: procedure; use arg v,label; if \v then raise syntax 88.900 array(label); return
assertFalse: procedure; use arg v,label; if v then raise syntax 88.900 array(label); return
::requires "crypto.cls"
