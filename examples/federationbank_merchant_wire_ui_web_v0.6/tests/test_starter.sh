...
set +e
"$ROOT/start.sh" --check --live >/tmp/fbm-starter-live.out 2>&1
rc=$?
set -e
[[ $rc -ne 0 ]]
grep -q 'Live mode requires' /tmp/fbm-starter-live.out
rm -f /tmp/fbm-starter-live.out
...
for _ in $(seq 1 100); do
  url="$(sed -n 's/^MERCHANT_WIRE_UI_URL=//p' "$log" | tail -1)"
  [[ -n "$url" ]] && break
  sleep 0.03
done
[[ -n "${url:-}" ]]
...
if(!t.includes('Merchant Bank') || !t.includes('NON-AUTHORITATIVE FRONT-END FIXTURE') || !t.includes('Merchant Banking overview')) process.exit(3);
