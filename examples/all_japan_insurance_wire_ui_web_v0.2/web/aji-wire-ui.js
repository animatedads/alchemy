...
const configNode = document.getElementById('aji-wire-ui-config');
...
function requireConfig(config) {
  for (const name of ['moduleUrl', 'gatewayUrl', 'outboundQueue']) {
    if (!config?.[name]) throw new Error(`AJI Wire UI bootstrap requires ${name}`);
  }
  for (const name of ['applicationId', 'sessionId', 'accessPointId']) {
    if (!config?.ownership?.[name]) throw new Error(`AJI Wire UI bootstrap requires ownership.${name}`);
  }
  return config;
}
...
async function boot() {
  ...
  const profile = new RenderProfileController({
    comms, definitions, definitionCache, renderer,
    siteId: config.siteId ?? 'ALL_JAPAN_INSURANCE'
  });
  const runtime = new WireUIRuntime({
    comms, definitions, renderer, serverSemantic: true,
    renderProfile: profile, ownership: config.ownership
  });
  ...
  globalThis.__allJapanInsuranceWireUI = Object.freeze({ comms, runtime, renderer, definitions, profile, material });
}

boot().catch((error) => {
  console.error('All Japan Insurance Wire UI bootstrap failed', error);
  status('Unable to connect', 'error');
  mount.replaceChildren();
  const panel = document.createElement('section');
  panel.className = 'wire-ui-bootstrap-error';
  panel.textContent = 'The All Japan Insurance workspace could not establish this authoritative session. Please reload or contact support.';
