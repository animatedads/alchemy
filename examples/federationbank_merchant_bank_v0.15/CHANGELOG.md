...
- Adds `MBSettlementInstructionEvidence`, keeping settlement instruction/dispatch distinct from actual settlement.
- Adds immutable `MBSettlementObservationEvidence` with `PARTIAL`, `COMPLETE` and `FAILED` outcomes bound to the exact obligation, instruction, debtor, creditor and currency.
- Adds cumulative partial-settlement tracking at the Merchant-domain truth layer; an obligation reaches `SETTLED` only when externally observed cumulative settlement exactly equals its contractual amount.
- A failed instruction does not extinguish the obligation and a new separately attributable instruction may be recorded for the remaining amount.
- Adds `MBSettlementStatusAssessment` checkpoints retaining the current settled and remaining amounts without converting operational work into settlement authority.
- Prevents over-settlement, wrong-party/wrong-direction evidence, unbound observations and concurrent open instructions.
- Deduplicates external settlement evidence by `(sourceAuthority, externalSettlementRef)` so the same settlement-agent receipt cannot be replayed under a new local evidence id.
- Adds five settlement regressions while preserving all v0.14 remediation execution-checkpoint semantics.
- No Retail/Core cash movement, Ledger posting, collateral movement or VMM implementation is added.

## v0.14

- Adds `MBHedgeRemediationPlanExecutionCheckpoint`: every observed remediation execution outcome can be bound to a transaction-time customer-rooted whole-book assessment instead of waiting for the final step.
- `PARTIAL` is now genuinely non-terminal. Multiple attributable observations may accumulate against one approved step; cumulative quantity/base exposure is checked before a later `COMPLETED` result can satisfy the step.
- Adds immutable per-step execution-evidence streams while retaining the existing evidence-id lookup and latest-step projection semantics.
- Adds transaction-time `assessCFDHedgeBookAsAt()` support for execution checkpoints so a later trade cannot rewrite the book that existed when an earlier fill was observed.
- Compares every checkpoint with the approved plan's `peakAbsBaseExposure`; excessive transient exposure is `EXECUTION_DIVERGENCE / TRANSIENT_EXPOSURE_BREACH`.
- Binds execution outcomes to `MBHedgeRemediationPlanStepDispatchEvidence` when a dispatch was observed. A correct-looking outcome from another/unidentified instruction is `DISPATCH_BINDING_DIVERGENCE`.
- Retains wrong-way, unexpected quantity, unexpected FX, instrument/settlement-line, object-semantic, ordering and evidence/book mismatch diagnostics at checkpoint level.
- Final verification counts prior divergent checkpoints. A later net-zero book cannot launder an earlier wrong-way/transient execution breach; the plan remains `DEVIATED` with `PRIOR_EXECUTION_CHECKPOINT_DIVERGENCE` when no stronger final cause applies.
- Adds regressions for partial-to-complete accumulation, dispatch/outcome binding and historical transient-divergence retention.

## v0.13

- Adds separate `MBHedgeRemediationPlanStepDispatchEvidence`: an attributable dispatch observation starts the execution lifecycle but is not a fill/result and cannot complete or cure remediation.
- All non-matching execution assessments now expose top-level `EXECUTION_DIVERGENCE`; `divergenceReason` retains machine-readable causes such as `WRONG_WAY_EXECUTION`, `UNEXPECTED_INSTRUMENT_OR_SETTLEMENT_LINE`, `UNEXPECTED_QUANTITY`, `UNEXPECTED_FX_RESULT`, failed/partial/not-executed outcomes, evidence/book mismatch and final book divergence.
- Adds explicit wrong-way/magnitude/instrument diagnostic counters while preserving compatibility with the original v0.13 verification constructor shape for read-only adapters.
- Plan steps may bind optional `expectedFxEvidenceRef`; terminal execution evidence may bind `observedFxEvidenceRef`, allowing unexpected FX results to be classified separately from unexpected quantity/base-exposure magnitude.
