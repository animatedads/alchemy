...
The v0.13 execution lifecycle is deliberately evidence-separated:

```text
approved plan step (intent)
        -> attributable dispatch observation (optional evidence)
        -> attributable terminal action observation
        -> fresh customer-rooted whole-book assessment
        -> execution assessment against approved projection
```

`MBHedgeRemediationPlanStepDispatchEvidence` proves only that an external execution channel reported dispatch. It does **not** prove a trade, custody movement, collateral movement, settlement or Core instruction occurred. Terminal `MBHedgeRemediationPlanStepExecutionEvidence` remains a separate fact and may report `COMPLETED`, `PARTIAL`, `FAILED` or `NOT_EXECUTED`.

A matched assessment is `MATCHED_PROJECTED_BOOK`. Any proved mismatch is top-level `EXECUTION_DIVERGENCE`, with a separate machine-readable `divergenceReason`. Quantity/base-exposure magnitude and FX are distinct diagnostics: `UNEXPECTED_QUANTITY` is used when the signed direction is correct but magnitude differs without an FX-evidence mismatch; `UNEXPECTED_FX_RESULT` is used when plan-bound expected FX evidence differs from observed execution FX evidence. In particular, an approved `+100` neutralisation that is actually observed as `-100` is `EXECUTION_DIVERGENCE / WRONG_WAY_EXECUTION`; the actual whole book, not the approved projection, remains authoritative. The plan becomes `DEVIATED`, and the underlying remediation remains open.


## v0.14 — execution checkpoints and partial-fill evidence

An approved plan is now checked throughout execution, not only after every step has a terminal result. Each `MBHedgeRemediationPlanStepExecutionEvidence` observation receives an immutable `MBHedgeRemediationPlanExecutionCheckpoint` against the customer-rooted book as it existed at that evidence's `effectiveAt`.

`PARTIAL` is non-terminal. A step may therefore produce several attributable fills whose signed base-exposure deltas accumulate. The step is complete only when terminal evidence arrives and the cumulative result matches the approved step semantics. A safe partial can remain `PARTIAL_WITHIN_APPROVED_ENVELOPE`; a wrong-way, oversized, wrongly routed, wrong-instrument or otherwise inconsistent partial is already `EXECUTION_DIVERGENCE` and does not have to wait for plan completion to become a risk fact.

The approved plan's `peakAbsBaseExposure` is an execution constraint as well as a planning disclosure. If the actual transaction-time book exceeds that envelope, the checkpoint records `TRANSIENT_EXPOSURE_BREACH`. Historical divergent checkpoints remain immutable evidence. Even if later activity happens to restore the final book to net zero, final verification cannot erase the earlier breach.

Dispatch and outcome causality is explicit. If an external dispatch was observed for a step, result evidence must bind itself to that exact `dispatchEvidenceRef`. Dispatch evidence still does not prove a fill, and Risk Service still has no live execution authority.


## v0.15 — attributable settlement evidence

Merchant settlement now has an explicit evidence lifecycle after a contractual `MBSettlementObligation` exists:
...
MBSettlementInstructionEvidence
      |     (instruction / dispatch evidence only)
      v
MBSettlementObservationEvidence
      ...
      |
      v
MBSettlementStatusAssessment
```

An instruction never settles an obligation. `PARTIAL` observations accumulate against the exact instruction and contractual obligation. `COMPLETE` must exactly complete the instructed amount; the obligation reaches `SETTLED` only when cumulative externally observed settlement equals the contractual obligation amount. A failed instruction leaves the obligation live and permits a separately attributable replacement instruction.

Debtor, creditor, currency, instruction identity and amount are checked before state can advance. The same external settlement receipt cannot be replayed under a different local evidence id: Merchant deduplicates the pair `(sourceAuthority, externalSettlementRef)` as well as the local evidence id.

These objects record externally attributable settlement facts only. They do not send a payment, mutate a Retail/Core Banking account, post a Core Ledger entry, move collateral or grant Risk Service settlement authority.
