...
const configNode = document.getElementById('flylo-wire-ui-config');
...
function requireConfig(config) {
  ...
  for (const name of required) if (!config?.[name]) throw new Error(`FlyLo Wire UI bootstrap requires ${name}`);
  for (const name of ['applicationId', 'sessionId', 'accessPointId']) {
    if (!config?.ownership?.[name]) throw new Error(`FlyLo Wire UI bootstrap requires ownership.${name}`);
  }
  return config;
}
...
async function boot() {
  ...
  const profile = new RenderProfileController({
    comms, definitions, definitionCache, renderer, siteId: config.siteId ?? 'FLYLO'
  });
  ...
  globalThis.__flyloWireUI = Object.freeze({ comms, runtime, renderer, definitions, profile, material });
}

boot().catch((error) => {
  console.error('FlyLo Wire UI bootstrap failed', error);
  status('Unable to connect', 'error');
  mount.replaceChildren();
  const panel = document.createElement('section');
  panel.className = 'wire-ui-bootstrap-error';
  panel.textContent = 'FlyLo could not establish this browser session. Please reload or try again.';
