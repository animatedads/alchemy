...
test('bootstrap URL obtains authoritative session binding without local queue/session copies', async()=>{
  ...
  const fetchImpl=async(url,options)=>{
    ...
    return {ok:true,status:200,json:async()=>({
      ...
      gatewayUrl:'wss://flylo.example/wire-ui?token=t',
      outboundQueue:'WIREUI.IN.AP-42',
      siteId:'FLYLO',
      ownership:{applicationId:'FLYLO-APP',sessionId:'SESSION-42',accessPointId:'AP-42'}
    })};
  };
  ...
  assert.equal(config.outboundQueue,'WIREUI.IN.AP-42');
