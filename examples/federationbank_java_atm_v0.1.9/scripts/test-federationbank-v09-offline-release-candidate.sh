...
patch -d "$work" -p1 < "$ROOT/integration/oorexx/federationbank_v0.9_atm_offline_release.patch"
...
qroot="$(mktemp -d)"
FB_TEST_QUEUE_ROOT="$qroot" rexx "$ROOT/integration/oorexx/test_federationbank_v09_offline_release_candidate.rex"
rm -rf "$qroot"
