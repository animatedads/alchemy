...
MODE="${STAFF_WIRE_UI_MODE:-auto}"
HOST="${STAFF_WIRE_UI_HOST:-127.0.0.1}"
PORT="${STAFF_WIRE_UI_PORT:-8080}"
BOOTSTRAP_URL="${STAFF_WIRE_UI_BOOTSTRAP_URL:-}"
BOOTSTRAP_FILE="${STAFF_WIRE_UI_BOOTSTRAP_FILE:-}"
...
Canonical package-root launcher for FederationBank Staff Wire UI.
...
Environment equivalents:
  ...
  STAFF_WIRE_UI_HOST
  STAFF_WIRE_UI_PORT
  STAFF_WIRE_UI_BOOTSTRAP_URL
  STAFF_WIRE_UI_BOOTSTRAP_FILE
...
[[ -f "$ROOT/tools/staff-dev-server.mjs" ]] || { echo 'starter runtime missing' >&2; exit 3; }

if [[ "$MODE" == auto ]]; then
  if [[ -n "$BOOTSTRAP_URL" || -n "$BOOTSTRAP_FILE" ]]; then MODE=live; else MODE=preview; fi
fi
if [[ "$MODE" != preview && "$MODE" != live ]]; then
  echo "STAFF_WIRE_UI_MODE must be auto, preview or live" >&2; exit 2
fi
...
if ((CHECK_ONLY)); then
  echo "FederationBank Staff Wire UI starter: PASS mode=$MODE"
  exit 0
fi
...
exec node "$ROOT/tools/staff-dev-server.mjs" "${args[@]}"
