...
const configNode = document.getElementById('staff-wire-ui-config');
function status(text, state = '') { statusNode.textContent = text; statusNode.dataset.state = state; }
function requireConfig(config) {
  for (const name of ['moduleUrl','gatewayUrl','outboundQueue']) if (!config?.[name]) throw new Error(`Staff Wire UI bootstrap requires ${name}`);
  for (const name of ['applicationId','sessionId','accessPointId']) if (!config?.ownership?.[name]) throw new Error(`Staff Wire UI bootstrap requires ownership.${name}`);
  return config;
}
async function boot() {
  ...
  const profile = new RenderProfileController({ comms, definitions, definitionCache, renderer, siteId: config.siteId ?? 'FEDERATIONBANK_STAFF_BANKING' });
  ...
  globalThis.__federationBankStaffWireUI = Object.freeze({ comms, runtime, renderer, definitions, profile, material });
}
boot().catch((error) => {
  console.error('FederationBank Staff Wire UI bootstrap failed', error);
  status('Unable to connect', 'error');
  mount.replaceChildren();
  const panel = document.createElement('section'); panel.className = 'wire-ui-bootstrap-error'; panel.textContent = 'The Staff Banking workspace could not establish this session. Please reload or contact support.'; mount.appendChild(panel);
});
