/* Actual Wire UI Builder v0.11 compiled package -> Swing composition acceptance. */
...
mf=.table~new; mf["manifestId"]="builder-v011"; mf["profileId"]="swing-large-fine"; mf["definitions"]=refs
bridge~accept(message("UI_RENDER_PROFILE",mf))
required=bridge~drainOutbound
if required~items<>1 then call fail "Builder profile cold request count" required~items
if required[1]["type"]<>"UI_DEFINITION_REQUIRED" then call fail "Builder profile cold request type" required[1]["type"]
if required[1]["definitions"]~items<>defs~items then call fail "Builder profile cold request definitions" required[1]["definitions"]~items
...
say "PASS Builder v0.11 compiled GRID12 -> ooRexx/BSF -> Swing widths=12,3,5,4,12"
exit 0
